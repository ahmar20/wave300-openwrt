...
/* Note: module test t1616 and subsystem tests 2301, 2302, 2401 and 2731 depend on the following fixed values */
		...
		#ifdef WAVE400_SINGLE_CPU
        #define NUM_STATION_DBASE_ENTRIES       64
		#else
        #define NUM_STATION_DBASE_ENTRIES       32
		#endif
...
// this struct contains data which is passed from Host to FW.
// location of this struct overlaps location of VECTOR_AREA.
// it will be override by VECTOR_AREA once the data is being read.
// note that even macro SET_DEBUG_CHI() will override it
typedef struct
{
    uint32 mipsWorkingFreq;
}__MTLK_PACKED VECTOR_AREA_INIT_DATA;
