...
_mtlk_df_nbuf_alloc_osdep (mtlk_vap_manager_t *vap_mgr, uint32 size)
{
...
_mtlk_df_nbuf_free_osdep (mtlk_vap_manager_t *vap_mgr, mtlk_nbuf_t *nbuf)
{
...
static __INLINE mtlk_nbuf_t *
_mtlk_df_nbuf_clone_no_priv_osdep (mtlk_vap_manager_t *vap_mgr, mtlk_nbuf_t *nbuf)
{
...
static __INLINE mtlk_nbuf_t *
_mtlk_df_nbuf_clone_with_priv_osdep (mtlk_vap_manager_t *vap_mgr, mtlk_nbuf_t *nbuf)
{
  mtlk_nbuf_t *res = skb_clone(nbuf, GFP_ATOMIC);
...
mtlk_map_to_phys_addr (mtlk_vap_manager_t    *vap_mgr,
                       void                  *buffer,
                       uint32                 size,
                       mtlk_data_direction_e  direction)
{
  ...
  ires = mtlk_vap_manager_get_master_vap(vap_mgr, &vap_handle);
  MTLK_ASSERT(MTLK_ERR_OK == ires);
  MTLK_UNREFERENCED_PARAM(ires);

  return dma_map_single(mtlk_vap_get_hw_vft(vap_handle)->get_device(vap_handle),
      buffer, size, direction);
}

static __INLINE void
mtlk_unmap_phys_addr (mtlk_vap_manager_t    *vap_mgr,
                      uint32                 addr,
                      uint32                 size,
                      mtlk_data_direction_e  direction)
{
  ...
  dma_unmap_single(mtlk_vap_get_hw_vft(vap_handle)->get_device(vap_handle),
      addr, size, direction);
}

#define mtlk_df_nbuf_map_to_phys_addr(vap_mgr, nbuf, size, direction) \
  mtlk_map_to_phys_addr(vap_mgr, nbuf->data, size, direction)

#define  mtlk_df_nbuf_unmap_phys_addr(vap_mgr, nbuf, addr, size, direction) \
  mtlk_unmap_phys_addr(vap_mgr, addr, size, direction)
