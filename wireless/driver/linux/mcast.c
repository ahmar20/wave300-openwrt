...
mtlk_mc_transmit (mtlk_core_t* nic, struct sk_buff *skb)
{
  ...
  for (; mcsta != NULL; mcsta = mcsta->next) {
    ...
    /* Note: Buffers are cloned with forced MTLK_NBUFF_RMCAST flag */
    res = mtlk_sq_enqueue_clone(nic, skb, mcsta->sta);
...
MTLK_MC_TRANSMIT_NONRMCAST:
  ...
  if ( (mtlk_nbuf_priv_check_flags(nbuf_priv, MTLK_NBUFF_BROADCAST) || 
        mtlk_nbuf_priv_check_flags(nbuf_priv, MTLK_NBUFF_MULTICAST)) && 
        mtlk_vap_is_ap(nic->vap_handle) &&
        MTLK_CORE_HOT_PATH_PDB_GET_INT(nic, CORE_DB_CORE_BRIDGE_MODE) == BR_MODE_WDS){
  
    wds_t       *wds;
    wds_peer_t  *peer;
    sta_entry   *sta;
    mtlk_dlist_entry_t *entry, *head;
    int         wds_sta_num, clone_res;

    clone_res = mtlk_sq_enqueue_clone_begin(nic->sq, ac, SQ_PUT_BACK, skb);
    if (MTLK_ERR_OK != clone_res){
      mtlk_core_inc_cnt(nic, MTLK_CORE_CNT_TX_PACKETS_DISCARDED_DRV_NO_RESOURCES);
    }
    ...
    mtlk_dlist_foreach(&wds->peers_list, entry, head){
      ...
      if (mtlk_nbuf_priv_get_src_sta(nbuf_priv) == sta ) continue; /* ignore sending to itself */

      /* STA is other than source STA*/
      /* For every peer_ap enqueue cloned packets to the back of the queue */
      /* Note: Buffers are cloned with forced MTLK_NBUFF_RMCAST flag */

      if (clone_res == MTLK_ERR_OK){
        /* Clone only if clone begun successfully*/

        ILOG3_Y("Broadcast pck cloned to peer AP %Y", mtlk_sta_get_addr(sta));

        res = mtlk_sq_enqueue_clone(nic, skb, sta);
  ...
  res = mtlk_sq_enqueue(nic, ac, SQ_PUT_BACK, skb);
  if (__LIKELY(MTLK_ERR_OK == res)) {
#ifndef MBSS_FORCE_NO_CHANNEL_SWITCH
    if (mtlk_vap_is_ap(nic->vap_handle)) {
      mtlk_aocs_on_tx_msdu_enqued(mtlk_core_get_master(nic)->slow_ctx->aocs, ac,
