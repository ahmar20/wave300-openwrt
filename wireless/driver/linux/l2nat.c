                                     ...
                                     mtlk_osal_timestamp_t *last_arp_req)
{
  ...
  uint32         arp_pck_size;
  mtlk_nbuf_priv_t *nbuf_priv_arp;
  ...
  nbuf_arp = mtlk_df_nbuf_alloc(mtlk_vap_get_manager(vap_handle), arp_pck_size);
  ...
  mtlk_df_nbuf_set_priority(nbuf_arp, mtlk_qos_get_tid_by_ac(AC_BE));

  /* Init private section */
  nbuf_priv_arp = mtlk_nbuf_priv(nbuf_arp);
  mtlk_nbuf_priv_set_vap_handle(nbuf_priv_arp, vap_handle);
  mtlk_nbuf_priv_set_flags(nbuf_priv_arp, MTLK_NBUFF_BROADCAST);

  /* Fill up ETH header */
  ether_header_arp = (struct ethhdr *)nbuf_arp->data;
  memcpy(ether_header_arp->h_source, ether_header_in->h_source, ETH_ALEN);
  ...
  memcpy(arp_data + 0,                             &ether_header_in->h_source, ETH_ALEN); /* Sender MAC - from IN pck */
  memset(arp_data + ETH_ALEN,                      0, IP_ALEN);                           /* Sender IP  - all 0 */
  memcpy(arp_data + ETH_ALEN + IP_ALEN,            &ether_header_in->h_dest, ETH_ALEN);   /* Target MAC - from IN pck */
  memcpy(arp_data + ETH_ALEN + IP_ALEN + ETH_ALEN, &ip_header_in->daddr, IP_ALEN);        /* Target IP  - from IN pck */

  /* Release original nbuf */
  mtlk_df_nbuf_free(mtlk_vap_get_manager(vap_handle), nbuf_in);
