...
int _mtlk_sq_send_to_hw(struct sk_buff *skb, uint16 prio)
{
  int res;
...
static int
mtlk_xmit_sq_enqueue (struct nic *nic, struct sk_buff *skb, struct net_device *dev, uint16 access_category, BOOL front)
{
  ...
  if (dst_sta != NULL) {
    ...
    sq_peer_ctx = &nic->sq_broadcast_ctx;
  }

  /* try to enqueue packet into the "send queue" */
  return mtlk_sq_enqueue_pkt(nic->sq, sq_peer_ctx, skb, access_category, front);
}
...
int mtlk_sq_enqueue_clone(struct nic *nic, mtlk_nbuf_t *nbuf, sta_entry *clone_dst_sta)
{
  ...
  clone_nbuf = mtlk_df_nbuf_clone_no_priv(sq->vap_mgr, nbuf);
  ...
  mtlk_nbuf_priv_set_flags(clone_nbuf_priv, MTLK_NBUFF_RMCAST);

  mtlk_nbuf_priv_extra_attach(nbuf_priv, clone_nbuf_priv);
  res = mtlk_xmit_sq_enqueue(nic,
                             clone_nbuf, 
                             clone_nbuf->dev, 
  ...
  if (unlikely(res != MTLK_ERR_OK)) {
    ...
    mtlk_df_nbuf_free(sq->vap_mgr, clone_nbuf);
  }
...
int mtlk_sq_enqueue(struct nic *nic, uint16 ac, BOOL front, struct sk_buff *skb)
{
  ...
  res = mtlk_xmit_sq_enqueue(nic, skb, skb->dev, ac, front);
  if (unlikely(res != MTLK_ERR_OK)) {
    mtlk_osal_atomic_dec(&sq->peer_queue[ac].size);
  }
