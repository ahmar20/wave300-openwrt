...
#include <asm/unaligned.h>
    ...
    #define PPA_SUCCESS                   IFX_SUCCESS
    #define LTQ_ETHSW_API_HANDLE          IFX_ETHSW_HANDLE
    #define ltq_ethsw_api_kopen(x)        ifx_ethsw_kopen(x)
    #define ltq_ethsw_api_kioctl(x, y, z) ifx_ethsw_kioctl(x, y, z)
    #define ltq_ethsw_api_kclose(x)       ifx_ethsw_kclose(x)
  #elif defined(CONFIG_LTQ_PPA_API_DIRECTPATH)
    /* UGW 6.1 and newer */
    #include <net/ppa_api.h>
    #include <net/ppa_hook.h>
    #include <asm/mach-lantiq/xway/switch-api/lantiq_ethsw_api.h>
  #else
    #error Invalid PPA configuration
  #endif
#endif /* MTLK_HAVE_PPA */
...
#define _DF_STAT_POLL_PERIOD   (125)
...
#define AHBUM_DATA_BASE     0x80000000
#define AHBUM_DATA_SIZE     0x00100000
#define LM_DATA_BASE        0x80080000
#define LM_DATA_SIZE        0x00018000
...
typedef struct _mtlk_df_user_ppa_t
{
  ...
  volatile BOOL            is_active; /** store internal PPA registration state. */
} mtlk_df_user_ppa_t;
#endif /* MTLK_HAVE_PPA */
...
struct _mtlk_df_user_t
{
  ...
  MTLK_DECLARE_INIT_LOOP(PROC_CARD_INIT);
...
MTLK_INIT_STEPS_LIST_BEGIN(df_user)
  ...
  MTLK_INIT_STEPS_LIST_ENTRY(df_user, PROC_CARD_INIT)
...
_mtlk_df_user_ppa_is_registered (mtlk_df_user_t* df_user)
{
  ...
  return (NULL != df_user->ppa.clb.rx_fn);
}
...
_mtlk_df_user_ppa_unregister (mtlk_df_user_t* df_user)
{
  ...
  if (ppa_res == PPA_SUCCESS) {
    ...
    /** \tocheck: PPA should not return error during unregistration.
     *    The PPA error have no sense for us during that state.
     *    Anyway we trying to keep the right internal state. */
    df_user->ppa.is_active = TRUE;
  }
}
...
static void
_mtlk_df_priv_xface_start (mtlk_df_user_t* df_user)
{
#ifdef MTLK_HAVE_PPA
  _mtlk_df_user_ppa_register(df_user);
#endif /* MTLK_HAVE_PPA */
}
                                ...
                                char *extra)
{
  ...
  if (MTLK_ERR_OK != res) {
    WLOG_SY("%s: Invalid address (%Y)", dev->name, mlme->addr.sa_data);
    goto finish;
  }
...
/*
  Parse string data for DELBA request

  \param delba_req - pointer to storage of DELBA configuration
  \param data      - string representation of DELBA configuration

  \return
     MTLK_ERR_PARAMS - incorrect format detected in string configuration
     MTLK_ERR_OK - success

  \remark
   available data format:
     "<MAC> <TID>" - send DELBA request for special TID
     "<MAC>"       - send DELBA request for all TIDs
*/
static int
_mtlk_df_user_fill_delba_req(char const *data, mtlk_delba_req_t *delba_req)
{
  if (NULL == data)
    return MTLK_ERR_PARAMS;

  if (MTLK_ERR_OK != mtlk_str_to_mac(data, delba_req->sDA.au8Addr))
  {
    ELOG_V("Wrong value for MAC");
    return MTLK_ERR_PARAMS;
  }
  ...
  if (NULL != data)
  {
    delba_req->tid = mtlk_osal_str_atol(data);

    if ((delba_req->tid < 0) && (delba_req->tid >= NTS_TIDS))
    {
      ELOG_D("Wrong value for tid [%d]", delba_req->tid);
      return MTLK_ERR_PARAMS;
    }
  }
  else
...
static int
_mtlk_df_user_fill_power_limit_cfg_by_int_vec(mtlk_tx_power_limit_cfg_t *cfg, uint16 intvec_length, int32 *intvec)
{
int res = MTLK_ERR_PARAMS;

  memset(cfg, 0, sizeof(mtlk_tx_power_limit_cfg_t));

  if (intvec_length != 3) {
    ELOG_V("Incorrect number of input values. Expected [11B boost] [BPSK boost] [auto-response boost]");
    return res;
  }

  if ((intvec[0] < MIN_INT8) || (intvec[0] > MAX_INT8) || (intvec[1] < MIN_INT8) || 
      (intvec[1] > MAX_INT8) || (intvec[2] < MIN_INT8) || (intvec[2] > MAX_INT8)) {

    ELOG_V("Incorrect input value");
    return res;
  }

  cfg->field_01 = intvec[0];
  cfg->field_02 = intvec[1];
  cfg->field_03 = intvec[2];
...
static int __MTLK_IFUNC
_mtlk_df_user_genl_int_get_cfg(mtlk_df_user_t* df_user, uint32 subcmd, char* data, uint16* length)
{
...
#define INTERF_THR_PARAMS_NUM       6
  ...
  if (INTERF_THR_PARAMS_NUM != intvec_length) {
    ...
    if ((i != INTERF_THR_PARAMS_SN_THR) && (((int32)intvec[i] < MIN_INT8) || ((int32)intvec[i] > 0))) {
...
static void
_mtlk_df_user_intvec_by_11b_antsel (uint32 *intvec, uint16 *intvec_length, const mtlk_11b_antsel_t *antsel)
{
...
static void
_mtlk_df_user_get_intvec_by_auto_params (uint32 *intvec, uint16 *intvec_lenth, const mtlk_coc_auto_cfg_t *auto_params)
{
  intvec[0] = auto_params->interval_1x1;
  intvec[1] = auto_params->interval_2x2;
  intvec[2] = auto_params->interval_3x3;
  intvec[3] = auto_params->high_limit_1x1;
  intvec[4] = auto_params->low_limit_2x2;
  intvec[5] = auto_params->high_limit_2x2;
  intvec[6] = auto_params->low_limit_3x3;

  *intvec_lenth = 7;
}

static void
_mtlk_df_user_get_intvec_by_antenna_params (uint32 *intvec, uint16 *intvec_lenth, const mtlk_coc_antenna_cfg_t *antenna_params)
{
...
static void
_mtlk_df_user_get_intvec_by_n_rate_bo (int32 *intvec, uint16 *intvec_lenth, const mtlk_core_n_rate_bo_t *n_rate_bo)
{
  intvec[0] = n_rate_bo->qam16;
  intvec[1] = n_rate_bo->qam64_2_3;
  intvec[2] = n_rate_bo->qam64_3_4;
  intvec[3] = n_rate_bo->qam64_5_6;

  *intvec_lenth = 4;
}

static void
_mtlk_df_user_get_intvec_by_interf_params (int32 *intvec, uint16 *intvec_lenth, const mtlk_interf_t *interf_cfg)
{
  intvec[0] = interf_cfg->is_enabled;
  intvec[1] = interf_cfg->threshold;
  intvec[2] = interf_cfg->threshold_rssi;
...
static void
_mtlk_df_user_get_intvec_by_pcoc_params (uint32 *intvec, uint16 *intvec_lenth, const mtlk_pcoc_params_t *params)
{
  intvec[0] = params->interval;
  intvec[1] = params->limit_lower;
  intvec[2] = params->limit_upper;
...
static void
_mtlk_df_user_intvec_by_11b_thresholds (uint32 *intvec, uint16 *intvec_length, const mtlk_enhanced11b_th_t *thresholds)
{
  intvec[0] = thresholds->Consecutive11nTH;
  intvec[1] = thresholds->Consecutive11bTH;

  *intvec_length = 2;
}

static void
_mtlk_df_user_intvec_by_recovery (uint32 *intvec, uint16 *intvec_length, const rcvry_cfg_t *recovery)
{
  intvec[0] = recovery->fast_rcvry_num;
  intvec[1] = recovery->full_rcvry_num;
  intvec[2] = recovery->complete_rcvry;
  intvec[3] = recovery->fail_time_interval;
  intvec[4] = recovery->fw_dump;
...
_mtlk_df_user_get_coc_auto_params_by_intvec (const uint32 *intvec, uint16 intvec_length, mtlk_coc_auto_cfg_t *auto_params)
{
  ...
  if (intvec_length != 7) {
    ...
    auto_params->high_limit_1x1 = intvec[3];
    auto_params->low_limit_2x2  = intvec[4];
    auto_params->high_limit_2x2 = intvec[5];
    auto_params->low_limit_3x3  = intvec[6];
...
static int
_mtlk_df_user_get_interf_params_by_intvec (const int32 *intvec, uint16 intvec_length, mtlk_interf_t *interf_cfg)
{
  int res = MTLK_ERR_PARAMS;

  memset(interf_cfg, 0, sizeof(mtlk_interf_t));
  ...
  if (!!intvec[0]) {
    if (intvec_length != 3) {
      ...
      if ((intvec[1] > 100) || (intvec[1] < 0)) {
        ELOG_D("Incorrect channel load threshold value (%d)", intvec[1]);
        return res;
      }

      if ((intvec[2] < -128) || (intvec[2] > 0)) {
        ELOG_D("Incorrect RSSI threshold value (%d)", intvec[2]);
        return res;
      }
    }
  }

  MTLK_CFG_SET_ITEM(interf_cfg, is_enabled, !!intvec[0]);
  if (!!intvec[0]) {
    MTLK_CFG_SET_ITEM(interf_cfg, threshold, intvec[1]);
    MTLK_CFG_SET_ITEM(interf_cfg, threshold_rssi, intvec[2]);
  }
...
_mtlk_df_user_get_pcoc_params_by_intvec (const uint32 *intvec, uint16 intvec_length, mtlk_pcoc_params_t *params)
{
  ...
  if (intvec_length != 3) {
    ...
    params->interval    = intvec[0];
    params->limit_lower = intvec[1];
    params->limit_upper = intvec[2];
...
static int
_mtlk_df_user_get_11b_thresholds_by_intvec (const uint32 *intvec, uint16 intvec_length, mtlk_enhanced11b_th_t *thresholds)
{
  int res = MTLK_ERR_PARAMS;
  ...
  if (intvec_length != 2) {
    ...
    thresholds->Consecutive11nTH = intvec[0];
    thresholds->Consecutive11bTH = intvec[1];
...
static int
_mtlk_df_user_get_recovery_by_intvec (const uint32 *intvec, uint16 intvec_length, rcvry_cfg_t *recovery)
{
  int res = MTLK_ERR_PARAMS;
  ...
  if (intvec_length != 5) {
    ...
    recovery->fast_rcvry_num = intvec[0];
    recovery->full_rcvry_num = intvec[1];
    recovery->complete_rcvry = intvec[2];
    recovery->fail_time_interval = intvec[3];
    recovery->fw_dump = intvec[4];
...
static void
_mtlk_df_user_intvec_by_agg_rate_limit (uint32 *intvec, uint16 *intvec_length, const mtlk_agg_rate_limit_t *limit)
{
  intvec[0] = limit->mode;
  intvec[1] = limit->maxRate;

  *intvec_length = 2;
}

/* Parameters:
    mode    0/1                 Feature Enable/Disable
    maxRate  0 ... BITRATE_LAST, use 0 if is not specified
*/
static int
_mtlk_df_user_set_agg_rate_limit_by_intvec (const uint32 *intvec, uint16 intvec_length, mtlk_agg_rate_limit_t *limit)
{
  int       res = MTLK_ERR_OK;
  uint32    mode, rate;
  ...
  if (0 == intvec_length) {
    ...
    mode = intvec[0];
    rate = (1 == intvec_length) ? 0 : intvec[1];

    /* check parameter 'mode' only */
    if (mode > 1) {
        ELOG_D("Wrong values for mode: %d (0/1 expected)\n", mode);
        res = MTLK_ERR_PARAMS;
    } else {
        limit->mode    = mode;
        limit->maxRate = rate;
...
*/
static int
_mtlk_df_user_set_rx_duty_cycle_by_intvec (const uint32 *intvec, uint16 intvec_length, mtlk_rx_duty_cycle_t *rx_duty_cycle)
{
  int       res = MTLK_ERR_OK;
  uint32    onTime, offTime;

  memset(rx_duty_cycle, 0, sizeof(*rx_duty_cycle));

  if (2 != intvec_length) {
    ELOG_V("Wrong parameter count (expected: 2)");
    res = MTLK_ERR_PARAMS;
  }
  else {
    onTime   = intvec[0];
    ...
    /* check parameters */
    if (onTime > 32767 || offTime > 32767) {
        ELOG_DD("Wrong values: %d %d (0...32767 expected)", onTime, offTime);
        res = MTLK_ERR_PARAMS;
    } else {
        rx_duty_cycle->onTime  = onTime;
        rx_duty_cycle->offTime = offTime;
        /* res = MTLK_ERR_OK; */
    }
  }
...
static void
_mtlk_df_user_intvec_by_rx_duty_cycle (uint32 *intvec, uint16 *intvec_length, const mtlk_rx_duty_cycle_t *rx_duty_cycle)
{
  intvec[0] = rx_duty_cycle->onTime;
  intvec[1] = rx_duty_cycle->offTime;
...
static int
_mtlk_df_user_set_lna_gain_by_intvec (const int32 *intvec, uint16 intvec_length, mtlk_lna_gain_t *lna_gains)
{
  int       res = MTLK_ERR_OK;

  memset(lna_gains, 0, sizeof(mtlk_lna_gain_t));

  if (2 != intvec_length) {
    ELOG_V("Incorrect input vector length. Expected parameters [extLnaGainBypass] [extLnaGainHigh]");
    return MTLK_ERR_PARAMS;
  }

  if ((intvec[0] < MIN_INT8) || (intvec[0] > MAX_INT8) || (intvec[1] < MIN_INT8) || (intvec[1] > MAX_INT8)) {
    ELOG_DD("Incorrect input value (%u %u)", intvec[0], intvec[1]);
    return MTLK_ERR_PARAMS;
  }

  lna_gains->lna_gain_bypass = (int8)intvec[0];
  lna_gains->lna_gain_high   = (int8)intvec[1];

  return res;
}

static void
_mtlk_df_user_intvec_by_lna_gain (int32 *intvec, uint16 *intvec_length, const mtlk_lna_gain_t *lna_gains)
{
  intvec[0] = lna_gains->lna_gain_bypass;
  intvec[1] = lna_gains->lna_gain_high;

  *intvec_length = 2;
}

static int
_mtlk_df_user_get_n_rate_bo_by_intvec (const int32 *intvec, uint16 intvec_length, mtlk_core_n_rate_bo_t *n_rate_bo)
{
  int res = MTLK_ERR_PARAMS;
  ...
  if (intvec_length != 4) {
    ...
    n_rate_bo->qam16     = intvec[0];
    n_rate_bo->qam64_2_3 = intvec[1];
    n_rate_bo->qam64_3_4 = intvec[2];
    n_rate_bo->qam64_5_6 = intvec[3];
...
_mtlk_df_user_do_fw_debug (const uint32 *intvec, uint16 intvec_length, UMI_FW_DBG_REQ *FWDebugReq)
{
  ...
  if ((1 > intvec_length) || (2 < intvec_length)) {
    ELOG_D("Incorrect vector length. length(%u)", intvec_length);
  } else {
    memset(FWDebugReq, 0, sizeof(UMI_FW_DBG_REQ));
    /* For some reason, first argument is ignored */
    if(2 == intvec_length) {
      FWDebugReq->debugType = intvec[1];
    }
    res = MTLK_ERR_OK;
  }
...
static void
_mtlk_df_user_addr_vect_to_text (char *buf, mtlk_clpb_t *addr_vect_clpb, uint16 *length, uint32 max_len)
{
...
_mtlk_df_user_print_ta_debug_info(char *buffer, size_t len, mtlk_ta_debug_info_cfg_t *debug_info)
{
  ...
  for (i=0; i < nof_crits; ++i) {
    ...
    length += mtlk_snprintf(buffer+length, len-length, " %02u %10X  %10X %10X %10X %4u %8u \n",
                       crit->id, crit->signature, (uint32)crit->fcn, (uint32)crit->clb,
  ...
  length += mtlk_snprintf(buffer+length, len-length,   "    STA    | VAP | Valid |   Prev    |    Delta    |    Prev    |   Delta     \n");
  length += mtlk_snprintf(buffer+length, len-length,   "------------------------------------------------------------------------------\n");
  for (i=0; i < nof_stas; ++i) {
    ...
    length += mtlk_snprintf(buffer+length, len-length, "%10X  %3X      %c     %10u    %10u   %10u   %10u\n",
                            (uint32)wss->sta_entry, (uint32)mtlk_vap_get_id(((sta_entry*)wss->sta_entry)->vap_handle),
...
static BOOL
_mtlk_df_user_card_type_is_g35(mtlk_df_user_t *df_user)
{
    ...
    vap = mtlk_df_get_vap_handle(df_user->df);
...
static BOOL
_mtlk_df_user_check_master_vap_on_g35(mtlk_df_user_t *df_user)
{
    return mtlk_vap_is_master(mtlk_df_get_vap_handle(df_user->df)) &&
           _mtlk_df_user_card_type_is_g35(df_user);
}

static int
__set_item_value_int8 (int32 value, int8 *item)
{
    int res = MTLK_ERR_OK;

    if (MIN_INT8 <= value && value <= MAX_INT8) {
        *item = value;
    } else {
        ELOG_V("Wrong value (expected: -128...127)");
        res = MTLK_ERR_PARAMS;
    }
...
_mtlk_df_user_iwpriv_get_core_param(mtlk_df_user_t* df_user, uint32 param_id, char* data, uint16* length)
{
  ...
  _DF_USER_GET_PARAM_MAP_START(df_user, param_id, res)
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_AOCS_RESTRICT_MODE, MTLK_CORE_REQ_GET_AOCS_CFG, FALSE, mtlk_aocs_cfg_t, aocs_cfg)
      MTLK_CFG_GET_ITEM(aocs_cfg, restrict_mode, *(uint32*)data);
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_AOCS_NON_WIFI_NOISE, MTLK_CORE_REQ_GET_AOCS_CFG, FALSE, mtlk_aocs_cfg_t, aocs_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(aocs_cfg, non_wifi_interf, _mtlk_df_user_get_intvec_by_interf_params,
                                     ((int32*)data, length, &aocs_cfg->non_wifi_interf));
    ...
    _DF_USER_GET_ON_PARAM(PRM_USE_SHORT_CYCLIC_PREFIX_RX, MTLK_CORE_REQ_GET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_GET_ITEM(mibs_cfg, short_cyclic_prefix_rx, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_USE_SHORT_CYCLIC_PREFIX_TX, MTLK_CORE_REQ_GET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_GET_ITEM(mibs_cfg, short_cyclic_prefix_tx, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_USE_SHORT_CYCLIC_PREFIX_RATE31, MTLK_CORE_REQ_GET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_GET_ITEM(mibs_cfg, short_cyclic_prefix_rate31, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_SHORT_PREAMBLE_OPTION_IMPLEMENTED, MTLK_CORE_REQ_GET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_GET_ITEM(mibs_cfg, short_preamble_option, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_SHORT_SLOT_TIME_OPTION_ENABLED_11G, MTLK_CORE_REQ_GET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
    ...
    _DF_USER_GET_ON_PARAM(MIB_TWO_ANTENNA_TRANSMISSION_ENABLE, MTLK_CORE_REQ_GET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      if (mtlk_vap_is_master(mtlk_df_get_vap_handle(df_user->df))) {
        MTLK_CFG_GET_ITEM(mibs_cfg, two_antenna_tx_enable, *(uint32*)data);
      } else {
          res = MTLK_ERR_NOT_SUPPORTED;
      }
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_PEERAP_KEY_IDX, MTLK_CORE_REQ_GET_WDS_CFG, FALSE, mtlk_wds_cfg_t, wds_cfg)
      MTLK_CFG_GET_ITEM(wds_cfg, peer_ap_key_idx, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_PEERAP_LIST, MTLK_CORE_REQ_GET_WDS_PEERAP, FALSE, mtlk_wds_cfg_t, wds_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(wds_cfg, peer_ap_vect, _mtlk_df_user_addr_vect_to_text, (data, wds_cfg->peer_ap_vect, length, TEXT_SIZE));
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_BSS_BASIC_RATE_SET, MTLK_CORE_REQ_GET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, core_cfg)
      MTLK_CFG_GET_ITEM(core_cfg, basic_rates, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_BSS_SUPPORTED_RATE_SET, MTLK_CORE_REQ_GET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, core_cfg)
      MTLK_CFG_GET_ITEM(core_cfg, supported_rates, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_BSS_EXTENDED_RATE_SET, MTLK_CORE_REQ_GET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, core_cfg)
      MTLK_CFG_GET_ITEM(core_cfg, extended_rates, *(uint32*)data);
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_RX_HIGH_THRESHOLD, MTLK_CORE_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg)
      MTLK_CFG_GET_ITEM(master_cfg, rx_high_threshold, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_CCA_THRESHOLD, MTLK_CORE_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg)
      MTLK_CFG_GET_ITEM(master_cfg, cca_threshold, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_RA_PROTECTION, MTLK_CORE_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg)
      MTLK_CFG_GET_ITEM(master_cfg, ra_protection, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_FORCE_NCB, MTLK_CORE_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg)
      MTLK_CFG_GET_ITEM(master_cfg, force_ncb, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_N_RATES_BOS, MTLK_CORE_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(master_cfg, n_rate_bo,
                                     _mtlk_df_user_get_intvec_by_n_rate_bo, ((int32*)data, length, &master_cfg->n_rate_bo));

    _DF_USER_GET_ON_PARAM(PRM_ID_DEBUG_TPC, MTLK_CORE_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg)
      MTLK_CFG_GET_ITEM(master_cfg, debug_tpc, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_VAP_LNA_GAINS, MTLK_CORE_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(master_cfg, lna_gain, _mtlk_df_user_intvec_by_lna_gain,
                                      ((uint32*)data, length, &master_cfg->lna_gain));
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_AGG_RATE_LIMIT, MTLK_CORE_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(master_cfg, agg_rate_limit, _mtlk_df_user_intvec_by_agg_rate_limit,
                                    ((uint32*)data, length, &master_cfg->agg_rate_limit));

    _DF_USER_GET_ON_PARAM(PRM_ID_RX_DUTY_CYCLE, MTLK_CORE_REQ_GET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(master_cfg, rx_duty_cycle, _mtlk_df_user_intvec_by_rx_duty_cycle,
                                     ((uint32*)data, length, &master_cfg->rx_duty_cycle));
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_TX_POWER_LIMIT, MTLK_CORE_REQ_GET_TX_POWER_LIMIT, FALSE,
                           mtlk_tx_power_limit_cfg_t, tx_power_limit_cfg)
      MTLK_CFG_GET_ITEM(tx_power_limit_cfg, field_01, ((int32*)data)[0]);
      MTLK_CFG_GET_ITEM(tx_power_limit_cfg, field_02, ((int32*)data)[1]);
      MTLK_CFG_GET_ITEM(tx_power_limit_cfg, field_03, ((int32*)data)[2]);
      *length = 3;
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_COC_POWER_MODE, MTLK_CORE_REQ_GET_COC_CFG, FALSE, mtlk_coc_mode_cfg_t, coc_cfg)
      MTLK_CFG_GET_ITEM(coc_cfg, is_auto_mode, *(uint32*)data);
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(coc_cfg, antenna_params,
                                     _mtlk_df_user_get_intvec_by_antenna_params, ((uint32*)data, length, &coc_cfg->antenna_params));

    _DF_USER_GET_ON_PARAM(PRM_ID_COC_AUTO_PARAMS, MTLK_CORE_REQ_GET_COC_CFG, FALSE, mtlk_coc_mode_cfg_t, coc_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(coc_cfg, auto_params,
                                     _mtlk_df_user_get_intvec_by_auto_params, ((uint32*)data, length, &coc_cfg->auto_params));

#ifdef MTCFG_PMCU_SUPPORT
    _DF_USER_GET_ON_PARAM(PRM_ID_PCOC_POWER_MODE, MTLK_CORE_REQ_GET_PCOC_CFG, FALSE, mtlk_pcoc_mode_cfg_t, pcoc_cfg)
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_PCOC_AUTO_PARAMS, MTLK_CORE_REQ_GET_PCOC_CFG, FALSE, mtlk_pcoc_mode_cfg_t, pcoc_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC_VOID(pcoc_cfg, params,
                                     _mtlk_df_user_get_intvec_by_pcoc_params, ((uint32*)data, length, &pcoc_cfg->params));
#endif
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_COEX_THRESHOLD, MTLK_CORE_REQ_GET_COEX_20_40_RSSI_THR_CFG, FALSE, mtlk_coex_20_40_mode_cfg_t, coex20_40_cfg)
      MTLK_CFG_GET_ITEM(coex20_40_cfg, rssi_threshold, *(int32*)data);
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_SCAN_AP_FORCE_SCAN_PARAMS_ON_ASSOC_STA, MTLK_CORE_REQ_GET_COEX_20_40_AP_FORCE_PARAMS_CFG, FALSE, mtlk_coex_20_40_ap_force_params_cfg_t, coex20_40_cfg)
      MTLK_CFG_GET_ITEM(coex20_40_cfg, ap_force_scan_params_on_assoc_sta, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_SCAN_AP_WAIT_FOR_SCAN_RESULTS_INTERVAL, MTLK_CORE_REQ_GET_COEX_20_40_AP_FORCE_PARAMS_CFG, FALSE, mtlk_coex_20_40_ap_force_params_cfg_t, coex20_40_cfg)
      MTLK_CFG_GET_ITEM(coex20_40_cfg, wait_for_scan_results_interval, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_EXEMPTION_REQ, MTLK_CORE_REQ_GET_COEX_20_40_STA_EXEMPTION_REQ_CFG, FALSE, mtlk_coex_20_40_exm_req_cfg_t, coex20_40_cfg)
      MTLK_CFG_GET_ITEM(coex20_40_cfg, exemption_req, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_MIN_NON_EXEMPTED_STA, MTLK_CORE_REQ_GET_COEX_20_40_AP_MIN_NUM_OF_EXM_STA_CFG, FALSE, mtlk_coex_20_40_min_num_exm_sta_cfg_t, coex20_40_cfg)
      MTLK_CFG_GET_ITEM(coex20_40_cfg, min_non_exempted_sta, *(uint32*)data);
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_SCAN_ACTIVITY_THRESHOLD, MTLK_CORE_REQ_GET_COEX_20_40_TIMES_CFG, FALSE, mtlk_coex_20_40_times_cfg_t, coex20_40_cfg)
      MTLK_CFG_GET_ITEM(coex20_40_cfg, scan_activity_threshold, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_SCAN_PASSIVE_DWELL, MTLK_CORE_REQ_GET_COEX_20_40_TIMES_CFG, FALSE, mtlk_coex_20_40_times_cfg_t, coex20_40_cfg)
      MTLK_CFG_GET_ITEM(coex20_40_cfg, scan_passive_dwell, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_SCAN_ACTIVE_DWELL, MTLK_CORE_REQ_GET_COEX_20_40_TIMES_CFG, FALSE, mtlk_coex_20_40_times_cfg_t, coex20_40_cfg)
      MTLK_CFG_GET_ITEM(coex20_40_cfg, scan_active_dwell, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_SCAN_PASSIVE_TOTAL_PER_CHANNEL, MTLK_CORE_REQ_GET_COEX_20_40_TIMES_CFG, FALSE, mtlk_coex_20_40_times_cfg_t, coex20_40_cfg)
      MTLK_CFG_GET_ITEM(coex20_40_cfg, scan_passive_total_per_channel, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_SCAN_ACTIVE_TOTAL_PER_CHANNEL, MTLK_CORE_REQ_GET_COEX_20_40_TIMES_CFG, FALSE, mtlk_coex_20_40_times_cfg_t, coex20_40_cfg)
      MTLK_CFG_GET_ITEM(coex20_40_cfg, scan_active_total_per_channel, *(uint32*)data);

    /* Interference Detection */
    _DF_USER_GET_ON_PARAM(PRM_ID_INTERFERENCE_MODE, MTLK_CORE_REQ_GET_INTERFDET_MODE_CFG, TRUE, mtlk_interfdet_mode_cfg_t, interfdet_mode_cfg)
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_AP_CAPABILITIES_MAX_ACLs, MTLK_CORE_REQ_GET_AP_CAPABILITIES, TRUE, mtlk_card_capabilities_t, card_capabilities)
      MTLK_CFG_GET_ITEM(card_capabilities, max_acls_supported, *(uint32*)data);
    ...
    _DF_USER_GET_ON_PARAM(PRM_ID_TA_TIMER_RESOLUTION, MTLK_CORE_REQ_GET_TA_CFG, FALSE, mtlk_ta_cfg_t, mtlk_ta_cfg)
      MTLK_CFG_GET_ITEM(mtlk_ta_cfg, timer_resolution, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_TA_DBG, MTLK_CORE_REQ_GET_TA_CFG, FALSE, mtlk_ta_cfg_t, mtlk_ta_cfg)
      MTLK_CFG_GET_ITEM_BY_FUNC(mtlk_ta_cfg, debug_info, _mtlk_df_user_print_ta_debug_info,
                                (data, TEXT_SIZE, &mtlk_ta_cfg->debug_info), *length);

    _DF_USER_GET_ON_PARAM(PRM_ID_ENHANCED11B_TH, MTLK_CORE_REQ_GET_ENHANCED11B_CFG, FALSE, mtlk_enhanced11b_cfg_t, mtlk_enhanced11b_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC_VOID(mtlk_enhanced11b_cfg, thresholds,
                                     _mtlk_df_user_intvec_by_11b_thresholds, ((uint32*)data, length, &mtlk_enhanced11b_cfg->thresholds));

    _DF_USER_GET_ON_PARAM(PRM_ID_FW_RECOVERY, MTLK_CORE_REQ_GET_RECOVERY_CFG, FALSE, mtlk_rcvry_cfg_t, fw_rcvry_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC_VOID(fw_rcvry_cfg, recovery_cfg,
                                     _mtlk_df_user_intvec_by_recovery, ((uint32*)data, length, &fw_rcvry_cfg->recovery_cfg));

    _DF_USER_GET_ON_PARAM(PRM_ID_11B_ANTENNA_SELECTION, MTLK_CORE_REQ_GET_11B_CFG, FALSE, mtlk_11b_cfg_t, mtlk_11b_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC_VOID(mtlk_11b_cfg, antsel,
                                     _mtlk_df_user_intvec_by_11b_antsel, ((uint32*)data, length, &mtlk_11b_cfg->antsel));

    /* 802.11w begin */
    _DF_USER_GET_ON_PARAM(PRM_ID_PMF_ACTIVATED,  MTLK_CORE_REQ_GET_DOT11W_CFG, FALSE, mtlk_dot11w_cfg_t, mtlk_dot11w_cfg)
      MTLK_CFG_GET_ITEM(mtlk_dot11w_cfg, pmf_activated,  *(uint32*)data);
    _DF_USER_GET_ON_PARAM(PRM_ID_PMF_REQUIRED,   MTLK_CORE_REQ_GET_DOT11W_CFG, FALSE, mtlk_dot11w_cfg_t, mtlk_dot11w_cfg)
      MTLK_CFG_GET_ITEM(mtlk_dot11w_cfg, pmf_required,   *(uint32*)data);
    _DF_USER_GET_ON_PARAM(PRM_ID_SAQ_RETR_TMOUT, MTLK_CORE_REQ_GET_DOT11W_CFG, FALSE, mtlk_dot11w_cfg_t, mtlk_dot11w_cfg)
      MTLK_CFG_GET_ITEM(mtlk_dot11w_cfg, saq_retr_tmout, *(uint32*)data);
    _DF_USER_GET_ON_PARAM(PRM_ID_SAQ_MAX_TMOUT,  MTLK_CORE_REQ_GET_DOT11W_CFG, FALSE, mtlk_dot11w_cfg_t, mtlk_dot11w_cfg)
      MTLK_CFG_GET_ITEM(mtlk_dot11w_cfg, saq_max_tmout,  *(uint32*)data);
    /* 802.11w end */

    _DF_USER_GET_ON_PARAM(PRM_ID_MC_PS_SIZE, MTLK_CORE_REQ_GET_MC_PS_SIZE_CFG, FALSE, mtlk_mc_ps_size_cfg_t, mc_ps_size_cfg)
      MTLK_CFG_GET_ITEM(mc_ps_size_cfg, maxNumberOfFsdus, *(uint32*)data);

    /* U-APSD mode */
    _DF_USER_GET_ON_PARAM(PRM_ID_UAPSD_MODE, MTLK_CORE_REQ_GET_UAPSD_MODE, FALSE, mtlk_uapsd_mode_cfg_t, uapsd_mode_cfg)
      MTLK_CFG_GET_ITEM(uapsd_mode_cfg, uapsd_enabled, *(uint32*)data);

    _DF_USER_GET_ON_PARAM(PRM_ID_UAPSD_MAX_SP, MTLK_CORE_REQ_GET_UAPSD_CFG, FALSE, mtlk_uapsd_cfg_t, uapsd_cfg)
      MTLK_CFG_GET_ITEM(uapsd_cfg, uapsd_max_sp, *(uint32*)data);
...
_mtlk_df_user_iwpriv_set_core_param(mtlk_df_user_t* df_user, uint32 param_id, char* data, uint16* length)
{
  ...
  _DF_USER_SET_PARAM_MAP_START(df_user, param_id, res)
    _DF_USER_SET_ON_PARAM(PRM_ID_DELBA_REQ, MTLK_CORE_REQ_SET_DELBA_REQ, FALSE, mtlk_delba_cfg_entity_t, delba_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(delba_cfg, delba_req, _mtlk_df_user_fill_delba_req,
                                ((char*)data, &delba_cfg->delba_req), res);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_AOCS_RESTRICT_MODE, MTLK_CORE_REQ_SET_AOCS_CFG, FALSE, mtlk_aocs_cfg_t, aocs_cfg)
    {
      if (*(uint32*)data >= AOCS_RESTRICT_CHAN_MODE_LAST) {
        res = MTLK_ERR_PARAMS;
      } else {
        MTLK_CFG_SET_ITEM(aocs_cfg, restrict_mode, *(uint32*)data);
      }
    }

    _DF_USER_SET_ON_PARAM(PRM_ID_AOCS_NON_WIFI_NOISE, MTLK_CORE_REQ_SET_AOCS_CFG, FALSE, mtlk_aocs_cfg_t, aocs_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(aocs_cfg, non_wifi_interf,
                                _mtlk_df_user_get_interf_params_by_intvec, ((int32*)data, *length, &aocs_cfg->non_wifi_interf), res);
    ...
    _DF_USER_SET_ON_PARAM(PRM_USE_SHORT_CYCLIC_PREFIX_RX, MTLK_CORE_REQ_SET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_SET_ITEM(mibs_cfg, short_cyclic_prefix_rx, !!(*(uint32*)data));

    _DF_USER_SET_ON_PARAM(PRM_USE_SHORT_CYCLIC_PREFIX_TX, MTLK_CORE_REQ_SET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_SET_ITEM(mibs_cfg, short_cyclic_prefix_tx, !!(*(uint32*)data));

    _DF_USER_SET_ON_PARAM(PRM_USE_SHORT_CYCLIC_PREFIX_RATE31, MTLK_CORE_REQ_SET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_SET_ITEM(mibs_cfg, short_cyclic_prefix_rate31, !!(*(uint32*)data));

    _DF_USER_SET_ON_PARAM(PRM_ID_SHORT_PREAMBLE_OPTION_IMPLEMENTED, MTLK_CORE_REQ_SET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      MTLK_CFG_SET_ITEM(mibs_cfg, short_preamble_option, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_SHORT_SLOT_TIME_OPTION_ENABLED_11G, MTLK_CORE_REQ_SET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
    ...
    _DF_USER_SET_ON_PARAM(MIB_TWO_ANTENNA_TRANSMISSION_ENABLE, MTLK_CORE_REQ_SET_MIBS_CFG, FALSE, mtlk_mibs_cfg_t, mibs_cfg)
      if (_mtlk_df_user_check_master_vap_on_g35(df_user)) {
        MTLK_CFG_SET_ITEM(mibs_cfg, two_antenna_tx_enable, *(uint32*)data);
      } else {
        res = MTLK_ERR_NOT_SUPPORTED;
      }
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_PEERAP_DBG, MTLK_CORE_REQ_SET_WDS_DBG, FALSE, mtlk_wds_dbg_t, wds_dbg)
      MTLK_CFG_SET_ITEM(wds_dbg, dummy, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_ADD_PEERAP, MTLK_CORE_REQ_SET_WDS_CFG, FALSE, mtlk_wds_cfg_t, wds_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_DEL_PEERAP, MTLK_CORE_REQ_SET_WDS_CFG, FALSE, mtlk_wds_cfg_t, wds_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_PEERAP_KEY_IDX, MTLK_CORE_REQ_SET_WDS_CFG, FALSE, mtlk_wds_cfg_t, wds_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_BSS_BASIC_RATE_SET, MTLK_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
      MTLK_CFG_SET_ITEM(pcore_cfg, basic_rates, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_BSS_SUPPORTED_RATE_SET, MTLK_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
      MTLK_CFG_SET_ITEM(pcore_cfg, supported_rates, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_BSS_EXTENDED_RATE_SET, MTLK_CORE_REQ_SET_CORE_CFG, FALSE, mtlk_gen_core_cfg_t, pcore_cfg)
      MTLK_CFG_SET_ITEM(pcore_cfg, extended_rates, *(uint32*)data);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_RX_HIGH_THRESHOLD, MTLK_CORE_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(master_core_cfg, rx_high_threshold, __set_item_value_int8,
                                (*(int32*)data, &master_core_cfg->rx_high_threshold), res);

    _DF_USER_SET_ON_PARAM(PRM_ID_CCA_THRESHOLD, MTLK_CORE_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(master_core_cfg, cca_threshold, __set_item_value_int8,
                                (*(int32*)data, &master_core_cfg->cca_threshold), res);

    _DF_USER_SET_ON_PARAM(PRM_ID_RA_PROTECTION, MTLK_CORE_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM(master_core_cfg, ra_protection, !!(*(uint32*)data));

    _DF_USER_SET_ON_PARAM(PRM_ID_FORCE_NCB, MTLK_CORE_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM(master_core_cfg, force_ncb, !!(*(uint32*)data));

    _DF_USER_SET_ON_PARAM(PRM_ID_N_RATES_BOS, MTLK_CORE_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, pmaster_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(pmaster_cfg, n_rate_bo,
                                _mtlk_df_user_get_n_rate_bo_by_intvec, ((int32*)data, *length, &pmaster_cfg->n_rate_bo), res);

    _DF_USER_SET_ON_PARAM(PRM_ID_DEBUG_TPC, MTLK_CORE_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM(master_core_cfg, debug_tpc, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_VAP_LNA_GAINS, MTLK_CORE_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, master_core_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(master_core_cfg, lna_gain, _mtlk_df_user_set_lna_gain_by_intvec,
                                ((uint32*)data, *length, &master_core_cfg->lna_gain), res);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_AGG_RATE_LIMIT, MTLK_CORE_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, pmaster_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(pmaster_cfg, agg_rate_limit, _mtlk_df_user_set_agg_rate_limit_by_intvec,
                                ((uint32*)data, *length, &pmaster_cfg->agg_rate_limit), res);

    _DF_USER_SET_ON_PARAM(PRM_ID_RX_DUTY_CYCLE, MTLK_CORE_REQ_SET_MASTER_CFG, FALSE, mtlk_master_core_cfg_t, pmaster_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(pmaster_cfg, rx_duty_cycle, _mtlk_df_user_set_rx_duty_cycle_by_intvec,
                                ((uint32*)data, *length, &pmaster_cfg->rx_duty_cycle), res);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_TX_POWER_LIMIT, MTLK_CORE_REQ_SET_HW_DATA_CFG, FALSE, mtlk_hw_data_cfg_t, hw_data_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(hw_data_cfg, power_limit, _mtlk_df_user_fill_power_limit_cfg_by_int_vec,
                                 (&hw_data_cfg->power_limit, *length, (int32*)data), res);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_COC_POWER_MODE, MTLK_CORE_REQ_SET_COC_CFG, FALSE, mtlk_coc_mode_cfg_t, coc_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_COC_AUTO_PARAMS, MTLK_CORE_REQ_SET_COC_CFG, FALSE, mtlk_coc_mode_cfg_t, coc_cfg)
...
#ifdef MTCFG_PMCU_SUPPORT
    _DF_USER_SET_ON_PARAM(PRM_ID_PCOC_POWER_MODE, MTLK_CORE_REQ_SET_PCOC_CFG, FALSE, mtlk_pcoc_mode_cfg_t, pcoc_cfg)
      MTLK_CFG_SET_ITEM(pcoc_cfg, is_enabled, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_PCOC_AUTO_PARAMS, MTLK_CORE_REQ_SET_PCOC_CFG, FALSE, mtlk_pcoc_mode_cfg_t, pcoc_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(pcoc_cfg, params,
                                _mtlk_df_user_get_pcoc_params_by_intvec, ((uint32*)data, *length, &pcoc_cfg->params), res);

    _DF_USER_SET_ON_PARAM(PRM_ID_PCOC_PMCU_DEBUG, MTLK_CORE_REQ_SET_PCOC_CFG, FALSE, mtlk_pcoc_mode_cfg_t, pcoc_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_COEX_THRESHOLD, MTLK_CORE_REQ_SET_COEX_20_40_RSSI_THR_CFG, FALSE, mtlk_coex_20_40_mode_cfg_t, coex20_40_cfg)
      MTLK_CFG_SET_ITEM(coex20_40_cfg, rssi_threshold, *(int32*)data);
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_SCAN_AP_FORCE_SCAN_PARAMS_ON_ASSOC_STA, MTLK_CORE_REQ_SET_COEX_20_40_AP_FORCE_PARAMS_CFG, FALSE, mtlk_coex_20_40_ap_force_params_cfg_t, coex20_40_cfg)
      MTLK_CFG_SET_ITEM(coex20_40_cfg, ap_force_scan_params_on_assoc_sta, *(uint32*)data);
    /*ILOG0_D("FUNCTION Value in core = %d\n", *(uint32*)data);*/

    _DF_USER_SET_ON_PARAM(PRM_ID_SCAN_AP_WAIT_FOR_SCAN_RESULTS_INTERVAL, MTLK_CORE_REQ_SET_COEX_20_40_AP_FORCE_PARAMS_CFG, FALSE, mtlk_coex_20_40_ap_force_params_cfg_t, coex20_40_cfg)
      MTLK_CFG_SET_ITEM(coex20_40_cfg, wait_for_scan_results_interval, *(uint32*)data);
    /*ILOG0_D("FUNCTION Value in core = %d\n", *(uint32*)data);*/

    _DF_USER_SET_ON_PARAM(PRM_ID_EXEMPTION_REQ, MTLK_CORE_REQ_SET_COEX_20_40_STA_EXEMPTION_REQ_CFG, FALSE, mtlk_coex_20_40_exm_req_cfg_t, coex20_40_cfg)
      MTLK_CFG_SET_ITEM(coex20_40_cfg, exemption_req, *(uint32*)data);
      /*ILOG0_D("FUNCTION Value in core = %d\n", *(uint32*)data);*/

    _DF_USER_SET_ON_PARAM(PRM_ID_MIN_NON_EXEMPTED_STA, MTLK_CORE_REQ_SET_COEX_20_40_AP_MIN_NUM_OF_EXM_STA_CFG, FALSE, mtlk_coex_20_40_min_num_exm_sta_cfg_t, coex20_40_cfg)
      MTLK_CFG_SET_ITEM(coex20_40_cfg, min_non_exempted_sta, *(uint32*)data);
      /*ILOG0_D("FUNCTION Value in core = %d\n", *(uint32*)data);*/
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_SCAN_ACTIVITY_THRESHOLD, MTLK_CORE_REQ_SET_COEX_20_40_TIMES_CFG, FALSE, mtlk_coex_20_40_times_cfg_t, coex20_40_cfg)
      MTLK_CFG_SET_ITEM(coex20_40_cfg, scan_activity_threshold, *(uint32*)data);
      /*ILOG0_D("FUNCTION Value in core = %d\n", *(uint32*)data);*/

    _DF_USER_SET_ON_PARAM(PRM_ID_SCAN_PASSIVE_DWELL, MTLK_CORE_REQ_SET_COEX_20_40_TIMES_CFG, FALSE, mtlk_coex_20_40_times_cfg_t, coex20_40_cfg)
      MTLK_CFG_SET_ITEM(coex20_40_cfg, scan_passive_dwell, *(uint32*)data);
    /*ILOG0_D("FUNCTION Value in core = %d\n", *(uint32*)data);*/

    _DF_USER_SET_ON_PARAM(PRM_ID_SCAN_ACTIVE_DWELL, MTLK_CORE_REQ_SET_COEX_20_40_TIMES_CFG, FALSE, mtlk_coex_20_40_times_cfg_t, coex20_40_cfg)
      MTLK_CFG_SET_ITEM(coex20_40_cfg, scan_active_dwell, *(uint32*)data);
    /*ILOG0_D("FUNCTION Value in core = %d\n", *(uint32*)data);*/

    _DF_USER_SET_ON_PARAM(PRM_ID_SCAN_PASSIVE_TOTAL_PER_CHANNEL, MTLK_CORE_REQ_SET_COEX_20_40_TIMES_CFG, FALSE, mtlk_coex_20_40_times_cfg_t, coex20_40_cfg)
      MTLK_CFG_SET_ITEM(coex20_40_cfg, scan_passive_total_per_channel, *(uint32*)data);
    /*ILOG0_D("FUNCTION Value in core = %d\n", *(uint32*)data);*/

    _DF_USER_SET_ON_PARAM(PRM_ID_SCAN_ACTIVE_TOTAL_PER_CHANNEL, MTLK_CORE_REQ_SET_COEX_20_40_TIMES_CFG, FALSE, mtlk_coex_20_40_times_cfg_t, coex20_40_cfg)
      MTLK_CFG_SET_ITEM(coex20_40_cfg, scan_active_total_per_channel, *(uint32*)data);
    /*ILOG0_D("FUNCTION Value in core = %d\n", *(uint32*)data);*/

    /* Interference Detection */
    _DF_USER_SET_ON_PARAM(PRM_ID_INTERFER_TIMEOUTS, MTLK_CORE_REQ_SET_INTERFDET_PARAMS_CFG, FALSE, mtlk_interfdet_cfg_t, interfdet_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_INTERFER_THRESH, MTLK_CORE_REQ_SET_INTERFDET_PARAMS_CFG, FALSE, mtlk_interfdet_cfg_t, interfdet_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_INTERFER_SCANTIMES, MTLK_CORE_REQ_SET_INTERFDET_PARAMS_CFG, FALSE, mtlk_interfdet_cfg_t, interfdet_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_DBG_CLI, MTLK_CORE_REQ_SET_DBG_CLI, FALSE, mtlk_dbg_cli_cfg_t, dbg_cli_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(dbg_cli_cfg, DbgCliReq,
                                _mtlk_df_user_do_simple_cli, ((uint32*)data, *length, &dbg_cli_cfg->DbgCliReq), res);

    _DF_USER_SET_ON_PARAM(PRM_ID_FW_DEBUG, MTLK_CORE_REQ_SET_FW_DEBUG, FALSE, mtlk_fw_debug_cfg_t, fw_debug_cfg)
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_FW_LOG_SEVERITY, MTLK_CORE_REQ_SET_FW_LOG_SEVERITY, FALSE, mtlk_fw_log_severity_t, fw_log_severity)
      if (2 != (*length)) {
    ...
    _DF_USER_SET_ON_PARAM(PRM_ID_TA_TIMER_RESOLUTION, MTLK_CORE_REQ_SET_TA_CFG, FALSE, mtlk_ta_cfg_t, mtlk_ta_cfg)
      MTLK_CFG_SET_ITEM(mtlk_ta_cfg, timer_resolution, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_ENHANCED11B_TH, MTLK_CORE_REQ_SET_ENHANCED11B_CFG, FALSE, mtlk_enhanced11b_cfg_t, mtlk_enhanced11b_cfg)
      MTLK_CFG_SET_ITEM_BY_FUNC(mtlk_enhanced11b_cfg, thresholds,
                                _mtlk_df_user_get_11b_thresholds_by_intvec, ((uint32*)data, *length, &mtlk_enhanced11b_cfg->thresholds), res);

  _DF_USER_SET_ON_PARAM(PRM_ID_11B_ANTENNA_SELECTION, MTLK_CORE_REQ_SET_11B_CFG, FALSE, mtlk_11b_cfg_t, mtlk_11b_cfg)
    ...
    /* 802.11w parameters begin */
    _DF_USER_SET_ON_PARAM(PRM_ID_PMF_ACTIVATED,  MTLK_CORE_REQ_SET_DOT11W_CFG, FALSE, mtlk_dot11w_cfg_t, mtlk_dot11w_cfg)
      MTLK_CFG_SET_ITEM(mtlk_dot11w_cfg, pmf_activated, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_PMF_REQUIRED,   MTLK_CORE_REQ_SET_DOT11W_CFG, FALSE, mtlk_dot11w_cfg_t, mtlk_dot11w_cfg)
      MTLK_CFG_SET_ITEM(mtlk_dot11w_cfg, pmf_required,  *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_SAQ_RETR_TMOUT, MTLK_CORE_REQ_SET_DOT11W_CFG, FALSE, mtlk_dot11w_cfg_t, mtlk_dot11w_cfg)
      MTLK_CFG_SET_ITEM(mtlk_dot11w_cfg, saq_retr_tmout, *(uint32*)data);

    _DF_USER_SET_ON_PARAM(PRM_ID_SAQ_MAX_TMOUT,  MTLK_CORE_REQ_SET_DOT11W_CFG, FALSE, mtlk_dot11w_cfg_t, mtlk_dot11w_cfg)
      MTLK_CFG_SET_ITEM(mtlk_dot11w_cfg, saq_max_tmout,  *(uint32*)data);
    /* 802.11w parameters end */

  _DF_USER_SET_ON_PARAM(PRM_ID_FW_RECOVERY, MTLK_CORE_REQ_SET_RECOVERY_CFG, FALSE, mtlk_rcvry_cfg_t, fw_rcvry_cfg)
  ...
  _DF_USER_SET_ON_PARAM(PRM_ID_MC_PS_SIZE, MTLK_CORE_REQ_SET_MC_PS_SIZE_CFG, FALSE, mtlk_mc_ps_size_cfg_t, mc_ps_size_cfg)
    MTLK_CFG_SET_ITEM(mc_ps_size_cfg, maxNumberOfFsdus, *(uint32*)data);

  /* U-APSD mode */
  _DF_USER_SET_ON_PARAM(PRM_ID_UAPSD_MODE, MTLK_CORE_REQ_SET_UAPSD_MODE, FALSE, mtlk_uapsd_mode_cfg_t, uapsd_mode_cfg)
    MTLK_CFG_SET_ITEM(uapsd_mode_cfg, uapsd_enabled, *(uint32*)data);

  _DF_USER_SET_ON_PARAM(PRM_ID_UAPSD_MAX_SP, MTLK_CORE_REQ_SET_UAPSD_CFG, FALSE, mtlk_uapsd_cfg_t, uapsd_cfg)
    MTLK_CFG_SET_ITEM(uapsd_cfg, uapsd_max_sp, *(uint32*)data);
...
_mtlk_df_user_unregister_ndev(mtlk_df_user_t *df_user, mtlk_vap_manager_interface_e intf)
{
  switch(intf) {
    ...
    case MTLK_VAP_SLAVE_INTERFACE:
      /* we cannot call free_netdev() directly, because unregistration will be complete only when we exit ioctl
...
mtlk_df_user_start(mtlk_df_t *df, mtlk_df_user_t *df_user, mtlk_vap_manager_interface_e intf)
{
  ...
  MTLK_START_TRY(df_user, MTLK_OBJ_PTR(df_user))
    ...
    MTLK_START_STEP_VOID_IF(mtlk_vap_is_master(mtlk_df_get_vap_handle(df_user->df)),
                         df_user, PPA_XFACE, MTLK_OBJ_PTR(df_user),
                         _mtlk_df_priv_xface_start, (df_user));
...
#ifdef MTLK_HAVE_PPA
  if (ppa_hook_directpath_send_fn && _mtlk_df_user_ppa_is_registered(df_user) && df_user->ppa.is_active) {
                                        ...
                                        size_t rsnie_len)
{
  ...
  MTLK_ASSERT(p - buf + rsnie_len*2 < IW_CUSTOM_MAX);
...
mtlk_df_ui_notify_notify_fw_hang(mtlk_df_t *df, uint32 fw_cpu, uint32 sw_watchdog_data)
{
  ...
  WLOG_DD("CID-%04x: FW CPU#%d hang detected", mtlk_vap_get_oid(mtlk_df_get_vap_handle(df)), fw_cpu);
}
...
mtlk_df_on_rcvry_isol (mtlk_df_t *df)
{
  ...
  mtlk_osal_event_set(&mtlk_df_get_user(df)->fw_hang_evts[UMIPS]);
  mtlk_osal_event_set(&mtlk_df_get_user(df)->fw_hang_evts[LMIPS]);
}
...
mtlk_df_on_rcvry_restore (mtlk_df_t *df)
{
  ...
  mtlk_osal_event_reset(&mtlk_df_get_user(df)->fw_hang_evts[UMIPS]);
  mtlk_osal_event_reset(&mtlk_df_get_user(df)->fw_hang_evts[LMIPS]);

  /* Start stat timer */
  mtlk_osal_timer_set(&mtlk_df_get_user(df)->slow_ctx->stat_timer, _DF_STAT_POLL_PERIOD);
}

/**************************************************************
 * Transactions with Core
 **************************************************************/
static int mtlk_df_ui_addba_status(mtlk_seq_entry_t *s, void *data)
{
  mtlk_df_t *df = mtlk_df_proc_seq_entry_get_df(s);
  mtlk_addba_peer_state_t *peer = NULL;
  mtlk_clpb_t *clpb = NULL;
  uint32 size;
  int i, res;
  ...
  res = _mtlk_df_user_invoke_core(df, MTLK_CORE_REQ_GET_ADDBA_STATE, &clpb, NULL, 0);
  res = _mtlk_df_user_process_core_retval_void(res, clpb, MTLK_CORE_REQ_GET_ADDBA_STATE, FALSE);

  if (MTLK_ERR_OK != res) {
    goto err_ret;
  }

  while(NULL != (peer = mtlk_clpb_enum_get_next(clpb, &size)))
  {
    MTLK_ASSERT(size == sizeof(*peer));

    mtlk_aux_seq_printf(s, MAC_PRINTF_FMT ":%s\n",
      MAC_PRINTF_ARG(peer->addr.au8Addr),
      peer->is_active ? "active" : "disabled");

    mtlk_aux_seq_printf(s, "  TX %4s %5s %9s %9s %s\n",
      "TID",
      "state",
      "req_cfmd",
      "req_dlgt",
    ...
    {
      mtlk_aux_seq_printf(s, "     %4d %5d %9u %9u %u\n",
        i,
        peer->tid[i].tx.state,
        peer->tid[i].tx.addba_req_cfmd_time,
        peer->tid[i].tx.addba_req_dlgt,
        peer->tid[i].tx.addba_res_rejects);
    }

    mtlk_aux_seq_printf(s, "  RX %4s %5s %10s %10s %s\n",
      "TID",
      "state",
      "del_tstamp",
      "req_tstamp",
    ...
    for (i = 0; i < ARRAY_SIZE(peer->tid); i++)
    {
      mtlk_aux_seq_printf(s, "     %4d %5d %10u %10u %u\n",
        i,
        peer->tid[i].rx.state,
        peer->tid[i].rx.delba_timeout,
        peer->tid[i].rx.req_tstamp,
        MAC_TO_HOST16(peer->tid[i].rx.addba_res.u16ResultCode)
      );
    }
  }
...
static int mtlk_df_ui_aocs_channels(mtlk_seq_entry_t *s, void *data)
{
  ...
  while(NULL != (stat = mtlk_clpb_enum_get_next(clpb, NULL))) {
    mtlk_aux_seq_printf(s," %3d %2d %3d %3d %5d %5d %5d %8d %4d %4d %8d\n",
                        stat->channel,
                        stat->sm_required,
                        stat->channel_load,
                        stat->num_20mhz_bss,
                        stat->num_40mhz_up_bss,
                        stat->num_40mhz_lw_bss,
                        stat->forty_mhz_intolerant,
                        stat->forty_mhz_int_affected,
                        stat->DrIf,
                        stat->FwIf,
                        stat->fl_noise_metric);
  }
...
#ifdef BT_ACS_DEBUG
static int mtlk_df_ui_aocs_channels_dbg(struct file *file, const char __user *buffer,
                                   unsigned long count, void *data)
{
  int res = MTLK_ERR_OK;
  mtlk_clpb_t *clpb = NULL;
  mtlk_df_t *df = (mtlk_df_t*) data;
  int num=0, bss20=0, bss40up=0, bss40lo=0, intol=0, noise=MIN_INT8;
  mtlk_aocs_channel_data_t entry;

  memset(&entry.stat, 0, sizeof(entry.stat));

  sscanf(buffer, "%d %d %d %d %d %d", &num, &bss20, &bss40up, &bss40lo, &intol, &noise);
  entry.stat.channel = num;

  if (num) {
    entry.stat.num_20mhz_bss        = bss20;
    entry.stat.num_40mhz_up_bss     = bss40up;
    entry.stat.num_40mhz_lw_bss     = bss40lo;
    entry.stat.forty_mhz_intolerant = intol;
    entry.stat.fl_noise_metric      = noise;
  }

  ILOG1_DDDDDD("Adding AOCS Channels Entry {channel=%d, bss20=%d, bss40up=%d, bss40lo=%d, intol=%d, noise=%d}", num, bss20, bss40up, bss40lo, intol, noise);

  res = _mtlk_df_user_invoke_core(df, MTLK_CORE_REQ_SET_AOCS_CHANNELS_TBL_DBG, &clpb, &entry, sizeof(entry));
  res = _mtlk_df_user_process_core_retval_void(res, clpb, MTLK_CORE_REQ_SET_AOCS_CHANNELS_TBL_DBG, TRUE);
...
#define ITF(head_str, ...)                  \
  ...
  while(NULL != (peer_status = mtlk_clpb_enum_get_next(clpb, NULL))) {
    BOOL uapsd_any = FALSE;

    /* Check if we have UAPSD enabled for any of the categories */
    for (idx = 0; idx < NTS_PRIORITIES; idx++) {
      if (peer_status->uapsd_enabled[idx]) {
        uapsd_any = TRUE;
        break;
      }
    }

    if (NULL == peer_status->vap_handle) { /* STA */
      mtlk_aux_seq_printf(s, "\nSTA MAC:" MAC_PRINTF_FMT, MAC_PRINTF_ARG(peer_status->mac_addr.au8Addr));
    } else {                    /* VAP */
      mtlk_aux_seq_printf(s, "\nVAP ID: %u", mtlk_vap_get_id(peer_status->vap_handle));
    }
    if (!peer_status->ps_mode_enabled || !uapsd_any) {
      mtlk_aux_seq_printf(s, "\nLimit: %10u\nUsed: %11u\n", peer_status->limit, peer_status->used);
    } else {
      const char *acs[] = {"BE","BK","VI","VO"};
      uint32 used;
      BOOL first;

      mtlk_aux_seq_printf(s, "\nLimits: Legacy(");
      for (first = TRUE, idx = 0; idx < NTS_PRIORITIES; idx++) {
        if (!peer_status->uapsd_enabled[idx]) {
          if (first) {
            first = FALSE;
          } else {
          ...
          mtlk_aux_seq_printf(s, "%s", acs[idx]);
        }
      }
      mtlk_aux_seq_printf(s, "):%02u", peer_status->limit);

      mtlk_aux_seq_printf(s, " U-APSD(");
      for (first = TRUE, idx = 0; idx < NTS_PRIORITIES; idx++) {
        if (peer_status->uapsd_enabled[idx]) {
          if (first) {
            first = FALSE;
      ...
      if (first) {
        ...
        mtlk_aux_seq_printf(s, "):%02u", peer_status->limit_uapsd);
      }
      mtlk_aux_seq_printf(s, "\nUsed:   Legacy(");
      for (first = TRUE, used = 0, idx = 0; idx < NTS_PRIORITIES; idx++) {
        if (!peer_status->uapsd_enabled[idx]) {
          if (first) {
            first = FALSE;
          } else {
          ...
          mtlk_aux_seq_printf(s, "%s", acs[idx]);
          used += peer_status->used_per_ac[idx];
        }
      }
      mtlk_aux_seq_printf(s, "):%02u", used);
      ...
      for (first = TRUE, idx = 0; idx < NTS_PRIORITIES; idx++) {
        if (peer_status->uapsd_enabled[idx]) {
          ...
          mtlk_aux_seq_printf(s, "%s:%02u", acs[idx], peer_status->used_per_ac[idx]);
        }
      }
      mtlk_aux_seq_printf(s, ")");
      mtlk_aux_seq_printf(s, "\n");
...
static int mtlk_df_ui_debug_power_save(mtlk_seq_entry_t *s, void *data)
{
  int                   res = MTLK_ERR_NOT_SUPPORTED;
  mtlk_clpb_t           *clpb = NULL;
  mtlk_df_t             *df = mtlk_df_proc_seq_entry_get_df(s);
  ...
  res = _mtlk_df_user_invoke_core(df, MTLK_CORE_REQ_GET_PS_STATUS, &clpb, NULL, 0);
  res = _mtlk_df_user_process_core_retval_void(res, clpb, MTLK_CORE_REQ_GET_PS_STATUS, FALSE);
...
#define ITF(head_str, ...)                      \
  ...
  ITF("    --------------------------", "%s", "-------------");
  ITF("    Station                  |", "    %s   |", mtlk_qos_get_ac_name(idx));
  ITF("    --------------------------", "%s", "-------------");

  num_of_elements = mtlk_clpb_get_num_of_elements(clpb);

  for (elem = 0; elem < num_of_elements-1; ++elem) {
  
    peer_status = mtlk_clpb_enum_get_next(clpb, NULL);
    if (NULL == peer_status) {
      res = MTLK_ERR_UNKNOWN;
      goto err_get_stat;
    }

    mtlk_aux_seq_printf(s, "    " MAC_PRINTF_FMT, MAC_PRINTF_ARG(peer_status->mac_addr.au8Addr));
    if (peer_status->ps_mode_enabled) {
      mtlk_aux_seq_printf(s, "  (PS)  |");
    } else {
      mtlk_aux_seq_printf(s, "        |");
    }
    for (idx = 0; idx < NTS_PRIORITIES; idx++) {
      mtlk_aux_seq_printf(s, "   %02d:%c:%02d  |", peer_status->used_per_ac[idx],
                          peer_status->uapsd_enabled[idx]?'U':'L', peer_status->limit_per_ac[idx]);
    }
    mtlk_aux_seq_printf(s,"\n");
  }
  
  ITF("    --------------------------", "%s", "-------------");
  mtlk_aux_seq_printf(s, "    Notes: <used>:<[U]-APSD|[L]egacy>:<limit>\n");
                               ...
                               unsigned long count, void *data)
{
  ...
  arg_num = sscanf(str, "%d", &arg_assert_type);
  if (arg_num < 1) {
    goto end;
  }

  if (arg_num >= 1) {
    if (arg_assert_type < 0 || arg_assert_type >= MTLK_CORE_UI_ASSERT_TYPE_LAST) {
      ELOG_DD("CID-%04x: Unsupported assert type: %d", mtlk_vap_get_oid(mtlk_df_get_vap_handle(df)), arg_assert_type);
      goto end;
    }
  }
  ...
  if (res == MTLK_ERR_NOT_READY) {
    ILOG0_D("CID-%04x: Already asserted", mtlk_vap_get_oid(mtlk_df_get_vap_handle(df)));
    goto end;
  }
  ...
  case MTLK_CORE_UI_ASSERT_TYPE_FW_LMIPS:
    ...
    break;
  case MTLK_CORE_UI_ASSERT_TYPE_FW_UMIPS:
    /* Wait for UMIPS to detect ASSERTION */
...
#ifdef MTCFG_BUS_AHB
static int mtlk_df_ui_ahbum(char *page, char **start, off_t off,
                             int count, int *eof, void *data)
{
  return _mtlk_df_ui_proc_bcl_read(page, start, off, count, eof, data, AHBUM_DATA_BASE, AHBUM_DATA_SIZE);
}
#endif /* MTCFG_BUS_AHB */
...
static int _mtlk_df_version_dump(mtlk_seq_entry_t *s, void *v)
{
  ...
  res = mtlk_vap_get_hw_vft(vap)->get_prop(vap, MTLK_HW_VERSION_PRINTOUT,
                                               buffer, MTLK_HW_VERSION_PRINTOUT_LEN);
...
#define PROC_NODE_ADDBA_STATUS "addba_status"

static int mtlk_df_ui_reg_addba_status(mtlk_df_user_t* df_user)
{
  return mtlk_df_proc_node_add_seq_entry(
            PROC_NODE_ADDBA_STATUS, df_user->slow_ctx->proc_df_debug_node, df_user->df, mtlk_df_ui_addba_status);
}

static void mtlk_df_ui_unreg_addba_status(mtlk_df_user_t* df_user)
{
  mtlk_df_proc_node_remove_entry(PROC_NODE_ADDBA_STATUS, df_user->slow_ctx->proc_df_debug_node);
}
...
#ifdef BT_ACS_DEBUG
static int mtlk_df_ui_reg_aocs_channels_dbg(mtlk_df_user_t* df_user)
{
  return mtlk_df_proc_node_add_wo_entry(
            "aocs_channels_dbg", df_user->slow_ctx->proc_df_node, df_user->df, mtlk_df_ui_aocs_channels_dbg);
}
#endif  /* BT_ACS_DEBUG */
...
static int mtlk_df_ui_reg_debug_power_save(mtlk_df_user_t* df_user)
{
  return mtlk_df_proc_node_add_seq_entry(
            "PS", df_user->slow_ctx->proc_df_debug_node, df_user->df, mtlk_df_ui_debug_power_save);
}
...
#ifdef MTCFG_BUS_AHB
static int mtlk_df_ui_reg_bcl_read_ahbum(mtlk_df_user_t* df_user)
{
  return mtlk_df_proc_node_add_ro_entry(
            "um", df_user->slow_ctx->proc_df_node, df_user->df, mtlk_df_ui_ahbum);
}
#endif /* MTCFG_BUS_AHB */
...
static int mtlk_df_ui_reg_version(mtlk_df_user_t* df_user)
{
  return mtlk_df_proc_node_add_seq_entry(
            "version", df_user->slow_ctx->proc_df_node, df_user->df, _mtlk_df_version_dump);
}
...
#ifdef BT_ACS_DEBUG
static void mtlk_df_ui_unreg_aocs_channels_dbg(mtlk_df_user_t* df_user)
{
  mtlk_df_proc_node_remove_entry("aocs_channels_dbg", df_user->slow_ctx->proc_df_node);
}
#endif  /* BT_ACS_DEBUG */
...
static void mtlk_df_ui_unreg_debug_power_save(mtlk_df_user_t* df_user)
{
  mtlk_df_proc_node_remove_entry("PS", df_user->slow_ctx->proc_df_debug_node);
}
...
#ifdef MTCFG_BUS_AHB
static void mtlk_df_ui_unreg_bcl_read_ahbum(mtlk_df_user_t* df_user)
{
  mtlk_df_proc_node_remove_entry("um", df_user->slow_ctx->proc_df_node);
}
#endif /* MTCFG_BUS_AHB */
...
static void mtlk_df_ui_unreg_version(mtlk_df_user_t* df_user)
{
  mtlk_df_proc_node_remove_entry("version", df_user->slow_ctx->proc_df_node);
}

#ifdef EEPROM_DATA_VALIDATION

#define PROC_EEPROM "eeprom"
...
_mtlk_df_ui_eeprom(mtlk_seq_entry_t *s, void *data)
{
  ...
  res = _mtlk_df_user_pull_core_data(df, MTLK_CORE_REQ_GET_EEPROM_CFG, FALSE,
...
static int mtlk_df_ui_reg_eeprom_proc_cl(mtlk_df_user_t* df_user)
{
  return mtlk_df_proc_node_add_seq_entry(PROC_EEPROM,
    df_user->slow_ctx->proc_df_node, df_user->df, _mtlk_df_ui_eeprom);
}

static void mtlk_df_ui_unreg_eeprom_proc_cl(mtlk_df_user_t* df_user)
{
  mtlk_df_proc_node_remove_entry(PROC_EEPROM, df_user->slow_ctx->proc_df_node);
}

#define PROC_EEPROMFW "eeprom_fw"

static int
_mtlk_df_ui_eepromfw(mtlk_seq_entry_t *s, void *data)
{
  int res = MTLK_ERR_NOT_SUPPORTED;
  uint32 req_data_length; \
  mtlk_df_t *df = mtlk_df_proc_seq_entry_get_df(s);
  ...
  res = _mtlk_df_user_pull_core_data(df, MTLK_CORE_REQ_GET_EEPROM_FW, FALSE,
                                     (void**) &eeprom_cfg, &req_data_length,
                                     &hdata);
...
static int mtlk_df_ui_reg_eepromfw_proc_cl(mtlk_df_user_t* df_user)
{
  return mtlk_df_proc_node_add_seq_entry(PROC_EEPROMFW,
    df_user->slow_ctx->proc_df_node, df_user->df, _mtlk_df_ui_eepromfw);
}

static void mtlk_df_ui_unreg_eepromfw_proc_cl(mtlk_df_user_t* df_user)
{
  mtlk_df_proc_node_remove_entry(PROC_EEPROMFW, df_user->slow_ctx->proc_df_node);
}
#endif /* EEPROM_DATA_VALIDATION */

struct _proc_management_handlers_t
{
  int  (*on_register)  (mtlk_df_user_t* df_user);
  void (*on_unregister)(mtlk_df_user_t* df_user);
};

static const struct _proc_management_handlers_t
_proc_management_handlers[] =
{
  ...
  { mtlk_df_ui_reg_addba_status,              mtlk_df_ui_unreg_addba_status},
  { mtlk_df_ui_reg_aocs_table,                mtlk_df_ui_unreg_aocs_table },
  { mtlk_df_ui_reg_aocs_channels,             mtlk_df_ui_unreg_aocs_channels },
#ifdef BT_ACS_DEBUG
  { mtlk_df_ui_reg_aocs_channels_dbg,         mtlk_df_ui_unreg_aocs_channels_dbg },
#endif  /* BT_ACS_DEBUG */
  ...
  { mtlk_df_ui_reg_debug_power_save,          mtlk_df_ui_unreg_debug_power_save },
  { mtlk_df_ui_reg_serializer_dump,           mtlk_df_ui_unreg_serializer_dump },
#ifdef EEPROM_DATA_VALIDATION
  { mtlk_df_ui_reg_eeprom_proc_cl,            mtlk_df_ui_unreg_eeprom_proc_cl },
  { mtlk_df_ui_reg_eepromfw_proc_cl,          mtlk_df_ui_unreg_eepromfw_proc_cl },
#endif /* EEPROM_DATA_VALIDATION */
  { mtlk_df_ui_reg_version,                   mtlk_df_ui_unreg_version },
};

#ifdef MTCFG_BUS_PCI_PCIE
static const struct _proc_management_handlers_t
_proc_management_pci_pcie_handlers[] =
{
...
#ifdef MTCFG_BUS_AHB
static const struct _proc_management_handlers_t
_proc_management_ahb_handlers[] =
{
  { mtlk_df_ui_reg_bcl_read_ahbum,           mtlk_df_ui_unreg_bcl_read_ahbum }
};
#endif /* MTCFG_BUS_AHB */
                                            ...
                                            char *extra)
{
  ...
  /* in case we don't add anything to the string we want an empty string */
  *str = '\0';
  ...
  if (!mtlk_df_is_ap(df_user->df)) {
    ...
    while (NULL != (stadb_stat = mtlk_clpb_enum_get_next(clpb, &size))) {
      if ((sizeof(*stadb_stat) != size) || (STAT_ID_STADB != stadb_stat->type)) {
        ...
        /* With 64 stas the total string may exceed 2046 bytes, so don't output too much here */
        len += snprintf(str + len, TEXT_SIZE - len, "\n" MAC_PRINTF_FMT, MAC_PRINTF_ARG(stadb_stat->u.general_stat.addr.au8Addr));
        
        if (len >= TEXT_SIZE)
        {
          WLOG_V("Connection information truncated");
          len = TEXT_SIZE - 1;
          break;
        }
      }
                               ...
                               char   *extra)
{
  ...
  enc_cfg.authentication = MAX_UINT8;
  ...
  if (0 != wrqu->data.length){
    ...
    memcpy(enc_cfg.wep_keys.sKey[0].au8KeyData, extra, wrqu->data.length);
  }

  res = _mtlk_df_user_invoke_core(df_user->df, MTLK_CORE_REQ_SET_WEP_ENC_CFG,
                                  &clpb, (char*)&enc_cfg, sizeof(enc_cfg));
  res = _mtlk_df_user_process_core_retval_void(res, clpb, MTLK_CORE_REQ_SET_WEP_ENC_CFG, TRUE);
  if(MTLK_ERR_OK != res) {
    goto finish;
  }
                                  ...
                                  char *extra)
{
  ...
  /* Set kernel's iwp->length here in order to prevent it to be equal to max_tokens,
     which leads to the errornous situation after this handler is called.
     TODO: check all other getters implementation. */
  encoding->length= sizeof(*ext);
...
pr_info("alg: %u\n", ext->alg);
...
#ifndef MTCFG_SUPORT_IW_ENCODE_ALG_NONE
    /* Because FW not support IW_ENCODE_ALG_NONE, just return MTLK_ERR_OK.*/
    res = MTLK_ERR_OK;
    goto finish;
#else
    /* Clear keys */
    if ((UMI_RSN_IGTK_GM_KEY_INDEX != key_idx) &&
        (UMI_RSN_IGTK_GN_KEY_INDEX != key_idx) &&
        (MIB_WEP_N_DEFAULT_KEYS < key_idx )) {
      WLOG_SD("%s: Invalid key index (%u)", dev->name, key_idx);
      goto finish;
    }
#endif
  } else if (ext->alg == IW_ENCODE_ALG_AES_CMAC) {
    /* Set IGTK keys */
    if(key_idx != UMI_RSN_IGTK_GM_KEY_INDEX &&
       key_idx != UMI_RSN_IGTK_GN_KEY_INDEX) {
      WLOG_SD("%s: Invalid IGTK key index (%u)", dev->name, key_idx);
      goto finish;
    }
  } else {
    /* Set normal keys */
    if (MIB_WEP_N_DEFAULT_KEYS < key_idx ) {
      WLOG_SD("%s: Invalid WEP key index (%u)", dev->name, key_idx);
      goto finish;
    }
  }
  if (0 != key_idx) {
  ...
  ILOG2_D("key: key_idx (%u)", key_idx);
  MTLK_CFG_SET_ITEM(&encext_cfg, key_idx, key_idx);

  /* Set default key index unless setting PMF IGTK key (index 4, 5) */
  if ((ext->ext_flags & IW_ENCODE_EXT_SET_TX_KEY) && (key_idx < MIB_WEP_N_DEFAULT_KEYS)) {
    MTLK_CFG_SET_ITEM(&encext_cfg, default_key_idx, key_idx);
    ILOG2_D("key: default_key_idx (%u)", key_idx);
  }
  ...
  if (ext->ext_flags & (IW_ENCODE_EXT_SET_TX_KEY|IW_ENCODE_EXT_GROUP_KEY)) {
    ...
    {
      if ((0 < key_len) && (key_len != MIB_WEP_KEY_WEP1_LENGTH) && (key_len != MIB_WEP_KEY_WEP2_LENGTH)) {
        WLOG_SD("%s: Invalid WEP key length (%u)", dev->name, key_len);
        res = MTLK_ERR_PARAMS;
        goto finish;
      }

      /* Prevent setting all-zero WEP key */
      if ((IW_ENCODE_ALG_WEP == ext->alg) &&
          (0 < key_len) &&
          (mtlk_df_ui_validate_wep_key(ext->key, key_len) != MTLK_ERR_OK)) {
        WLOG_S("%s: Invalid WEP key", dev->name);
        res = MTLK_ERR_PARAMS;
        goto finish;
      }
    }

    if ((UMI_RSN_TK1_LEN + UMI_RSN_TK2_LEN) < key_len) {
      WLOG_SD("%s: Invalid key length (%u)", dev->name, key_len);
      key_len = UMI_RSN_TK1_LEN + UMI_RSN_TK2_LEN;
    }
  }
  MTLK_CFG_SET_ITEM(&encext_cfg, key_len, key_len);
  ILOG2_D("key: key_len (%u)", key_len);

  /* Determine and validate the key */
  if (key_len) {
    MTLK_CFG_SET_ITEM_BY_FUNC_VOID(&encext_cfg, key, memcpy,
                                         (encext_cfg.key, ext->key, key_len));
    mtlk_dump(2, ext->key, key_len, "KEY:");
  }
                                    ...
                                    char *extra)
{
  ...
  res = _mtlk_df_ui_execute_cfg_handler(_mtlk_df_ui_cfg_setters_tbl,
                                            df_user, wrqu->data.flags, extra, &wrqu->data.length);
  return _mtlk_df_mtlk_to_linux_error_code(res);
}
...
_mtlk_df_user_cleanup(mtlk_df_user_t *df_user)
{
  ...
  MTLK_CLEANUP_BEGIN(df_user, MTLK_OBJ_PTR(df_user))
      ...
      CARD_SELECTOR_START(card_type)
        IF_CARD_G3 (
          for (i = 0; MTLK_CLEANUP_ITERATONS_LEFT(MTLK_OBJ_PTR(df_user), PROC_CARD_INIT) > 0; i++) {
            MTLK_CLEANUP_STEP_LOOP(df_user, PROC_CARD_INIT, MTLK_OBJ_PTR(df_user),
                                  _proc_management_pci_pcie_handlers[i].on_unregister, (df_user));
           }
        );
        IF_CARD_AHBG35 (
          for (i = 0; MTLK_CLEANUP_ITERATONS_LEFT(MTLK_OBJ_PTR(df_user), PROC_CARD_INIT) > 0; i++) {
            MTLK_CLEANUP_STEP_LOOP(df_user, PROC_CARD_INIT, MTLK_OBJ_PTR(df_user),
                                   _proc_management_ahb_handlers[i].on_unregister, (df_user));
          }
        );
      CARD_SELECTOR_END();
                   ...
                   struct net_device *dev)
{
  ...
  */
  ...
  MTLK_INIT_TRY(df_user, MTLK_OBJ_PTR(df_user))
    ...
    CARD_SELECTOR_START(card_type)
      IF_CARD_G3 (
        for (i = 0; i < ARRAY_SIZE(_proc_management_pci_pcie_handlers); i++) {
          MTLK_INIT_STEP_LOOP(df_user, PROC_CARD_INIT, MTLK_OBJ_PTR(df_user),
                              _proc_management_pci_pcie_handlers[i].on_register, (df_user));
        }
      );
      IF_CARD_AHBG35 (
        for (i = 0; i < ARRAY_SIZE(_proc_management_ahb_handlers); i++) {
          MTLK_INIT_STEP_LOOP(df_user, PROC_CARD_INIT, MTLK_OBJ_PTR(df_user),
                              _proc_management_ahb_handlers[i].on_register, (df_user));
        }
      );
    CARD_SELECTOR_END();
...
/* Remove MAC address from switch MAC table by ifx_ethsw_kioctl
     1. QUERY: search MAC address in switch table.
        The bFound will be set to 1 if MAC address found.
     2. REMOVE: remove MAC address if found.
   Note: It is should be done independently of whether PPA is active or not.
*/
void __MTLK_IFUNC
mtlk_df_user_ppa_remove_mac_addr(mtlk_df_t *df, const uint8 *mac_addr)
{
    ...
    IFX_ETHSW_MAC_tableQuery_t  MAC_tableQuery;
    IFX_ETHSW_MAC_tableRemove_t MAC_tableRemove;

    MTLK_ASSERT(NULL != df);
    MTLK_ASSERT(NULL != mac_addr);

    /* ifx_ethsw_kopen returns a zero in case the device does not exist or is blocked */
    handle = ltq_ethsw_api_kopen("/dev/switch/0");

    if (0 != handle) {

        /* 1. Search MAC address in switch table */
        memset(&MAC_tableQuery, 0x00, sizeof(MAC_tableQuery));
        MAC_tableQuery.nFId = 0;
        memcpy(MAC_tableQuery.nMAC, mac_addr, IEEE_ADDR_LEN);
        ltq_ethsw_api_kioctl(handle, IFX_ETHSW_MAC_TABLE_ENTRY_QUERY, (unsigned int)&MAC_tableQuery);

        ILOG3_YDD("MAC %Y switch query: bFound %d, nPortId %d",
            mac_addr, (int)MAC_tableQuery.bFound, (int)MAC_tableQuery.nPortId);

        /* 2. Remove MAC address if found */
        if (MAC_tableQuery.bFound) {
            memset(&MAC_tableRemove, 0x00, sizeof(MAC_tableRemove));
            MAC_tableRemove.nFId = 0;
            memcpy(MAC_tableRemove.nMAC, mac_addr, IEEE_ADDR_LEN);
            ltq_ethsw_api_kioctl(handle, IFX_ETHSW_MAC_TABLE_ENTRY_REMOVE, (unsigned int)&MAC_tableRemove);

            ILOG3_Y("MAC %Y removed from switch MAC table", mac_addr);
        }
