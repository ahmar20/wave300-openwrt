...
_mtlk_progmodel_init(mtlk_txmm_t *txmm, mtlk_progmodel_t *fw, struct nic *nic, int freq, int spectrum)
{
  ...
  if (MTLK_ERR_OK != mtlk_vap_get_hw_vft(txmm->vap_handle)->get_prop(
    txmm->vap_handle, MTLK_HW_PROP_CHIP_NAME, 
    &fw->prgmdl_params.chip_name,
    sizeof(&fw->prgmdl_params.chip_name)))
  {
    goto ERROR;
  }
...
mtlk_progmodel_free_os(mtlk_progmodel_t *fw)
{
  ...
  for (i = 0; i < MTLK_PRGMDL_TYPE_LAST; i++)
    mtlk_df_fw_unload_file(mtlk_vap_get_manager(fw->nic->vap_handle), &fw->ff[i].fcontents);
...
mtlk_progmodel_load_from_os(mtlk_progmodel_t *fw)
{
  ...
  for (i = 0; i < MTLK_PRGMDL_TYPE_LAST; i++) {
    ...
    res = mtlk_df_fw_load_file(mtlk_vap_get_manager(fw->nic->vap_handle), fw->ff[i].fname, &fw->ff[i].fcontents);
    ...
    fw->ff[i].update_version = (i == MTLK_PRGMDL_TYPE_HW) ? TRUE : FALSE;
  }
...
ERROR:
  ...
  for (t = 0; t < i; t++)
    mtlk_df_fw_unload_file(mtlk_vap_get_manager(fw->nic->vap_handle), &fw->ff[t].fcontents);
...
mtlk_progmodel_get_fname(mtlk_prgmdl_type_e prgmdl_type, mtlk_prgmdl_parse_t *param, char *fname)
{
  ...
  /* CB = spectrum_mode == SPECTRUM_40MHZ, nCB = spectrum_mode == MIB_SPECTRUM_20M */
  if (SPECTRUM_40MHZ == param->spectrum_mode)
    strcat(fname, "CB");
  ...
  if (MTLK_PRGMDL_TYPE_HW == prgmdl_type)
    sprintf(&fname[strlen(fname)], "_%02X_Rev%c", (int)param->hw_type, (char)param->hw_revision);
  else if (MTLK_PRGMDL_TYPE_PHY != prgmdl_type)
    result = MTLK_ERR_PARAMS;

  sprintf(&fname[strlen(fname)], "_%s.bin", param->chip_name);
