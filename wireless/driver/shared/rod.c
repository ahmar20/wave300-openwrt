...
#define MTLK_ROD_CNT_RX_PACKETS_DISCARDED_DRV_TOO_OLD_ALLOWED   MTLK_WWSS_WLAN_STAT_ID_RX_PACKETS_DISCARDED_DRV_TOO_OLD
#define MTLK_ROD_CNT_RX_PACKETS_DISCARDED_DRV_DUPLICATE_ALLOWED MTLK_WWSS_WLAN_STAT_ID_RX_PACKETS_DISCARDED_DRV_DUPLICATE_ALLOWED
...
mtlk_create_rod_queue (reordering_queue *prod_queue, mtlk_vap_handle_t vap_handle, mtlk_wss_t *parent_wss, int win_size, int ssn)
{
  ...
  prod_queue->last_sent    = mtlk_osal_timestamp();
  prod_queue->last_rx_time = mtlk_osal_timestamp_to_ms(prod_queue->last_sent);    /* in milliseconds */
...
#define _mtlk_rod_inc_cnt(queue, id)        { if (id##_ALLOWED) __mtlk_rod_inc_cnt(queue, id); }

static void
__mtlk_rod_inc_cnt (reordering_queue  *prod_queue,
                     ...
                     int seq, mtlk_nbuf_t *nbuf)
{
  ...
  if (diff > SEQUENCE_NUMBER_LIMIT/2) {
    ...
    mtlk_df_nbuf_free_sub_frames(mtlk_vap_get_manager(prod_queue->vap_handle), nbuf);
    goto end;
  }
