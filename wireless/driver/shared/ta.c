...
_ta_wss_cntr_update (_ta_wss_cntr_t *cntr, uint32 wss_curr)
{
  ...
  ILOG3_DDDD("TA WSS CNTR (0x%08X) old/new/delta %d/%d/%d", cntr, wss_prev, wss_curr, wss_delta);
...
/** Convert bytes per interval into Mibits per second */
static uint32 
...
static uint32
_pcoc_convert_result (uint32 tp_bytes_interval, uint32 coeff)
{
  return (uint32) (((uint64) tp_bytes_interval * (uint64) coeff) >> TA_Q_SHIFT);
}
...
_ta_crit_coc (_ta_crit_entry_t *crit)
{
  ...
  {
    sta_wss = MTLK_LIST_GET_CONTAINING_RECORD(entry, _sta_wss_entry_t, list_entry);
    ILOG3_Y("WSS STA %Y", mtlk_sta_get_addr(sta_wss->sta));

    val_rx = _ta_wss_cntr_update(&sta_wss->coc_rx_bytes, mtlk_sta_get_cnt(sta_wss->sta, MTLK_STAI_CNT_BYTES_RECEIVED));
    val_tx = _ta_wss_cntr_update(&sta_wss->coc_tx_bytes, mtlk_sta_get_cnt(sta_wss->sta, MTLK_STAI_CNT_BYTES_SENT));
  ...
  ILOG3_DD("CoC MAX_TP %d bytes, %d Mib/sec", max_tp_bytes, max_tp_mibits);
...
_ta_crit_coc_init (_ta_crit_entry_t *crit)
{
  ...
  mtlk_dlist_foreach(&ta->sta_wss_list, entry, head)
  {
    ...
    _ta_wss_cntr_update(&sta_wss->coc_rx_bytes, mtlk_sta_get_cnt(sta_wss->sta, MTLK_STAI_CNT_BYTES_RECEIVED));
    _ta_wss_cntr_update(&sta_wss->coc_tx_bytes, mtlk_sta_get_cnt(sta_wss->sta, MTLK_STAI_CNT_BYTES_SENT));
...
static void
_ta_crit_pcoc (_ta_crit_entry_t *crit)
{
  mtlk_dlist_entry_t  *entry;
  mtlk_dlist_entry_t  *head;
  _sta_wss_entry_t    *sta_wss;
  ...
  uint32  sum_tp_bytes, sum_tp_kbps;
  uint32  val_rx, val_tx;
  ...
  mtlk_dlist_foreach(&ta->sta_wss_list, entry, head)
  {
    ...
    val_rx = _ta_wss_cntr_update(&sta_wss->pcoc_rx_bytes, mtlk_sta_get_cnt(sta_wss->sta, MTLK_STAI_CNT_BYTES_RECEIVED));
    val_tx = _ta_wss_cntr_update(&sta_wss->pcoc_tx_bytes, mtlk_sta_get_cnt(sta_wss->sta, MTLK_STAI_CNT_BYTES_SENT));

    if (!sta_wss->pcoc_wss_valid){
      /* Skip processing not valid WSS */
      ILOG3_V("WSS STA skipped");
      sta_wss->pcoc_wss_valid = TRUE;
      continue;
    }
  ...
  sum_tp_kbps = _pcoc_convert_result(sum_tp_bytes, ta->pcoc_cfg.coeff);
  ILOG3_DD("PCoC SUM_TP %d bytes, %d kbps", sum_tp_bytes, sum_tp_kbps);

  crit->clb(crit->clb_ctx, HANDLE_T(&sum_tp_kbps));

  return;
}

static void
_ta_crit_pcoc_init (_ta_crit_entry_t *crit)
{
  mtlk_dlist_entry_t  *entry;
  mtlk_dlist_entry_t  *head;
  _sta_wss_entry_t    *sta_wss;
  ...
  ILOG2_V("PCOC Criterion Init");

  /* Init all PCoC WSS counters */
  mtlk_osal_mutex_acquire(&ta->sta_wss_list_lock);
  mtlk_dlist_foreach(&ta->sta_wss_list, entry, head)
  {
    sta_wss = MTLK_LIST_GET_CONTAINING_RECORD(entry, _sta_wss_entry_t, list_entry);

    _ta_wss_cntr_update(&sta_wss->pcoc_rx_bytes, mtlk_sta_get_cnt(sta_wss->sta, MTLK_STAI_CNT_BYTES_RECEIVED));
    _ta_wss_cntr_update(&sta_wss->pcoc_tx_bytes, mtlk_sta_get_cnt(sta_wss->sta, MTLK_STAI_CNT_BYTES_SENT));
...
/** PCoC configuration function */
int __MTLK_IFUNC
mtlk_ta_pcoc_cfg (mtlk_handle_t ta_handle, ta_crit_pcoc_cfg_t *cfg)
{
  _traf_analyzer_t *ta;
  _ta_crit_entry_t *crit;
  uint32  ta_tmr_res_ms;
  ...
  if ((cfg->interval * TA_PCOC_TICK_MSEC) < ta_tmr_res_ms) {
    WLOG_D("Value shouldn't be less than timer resolution %d msec",ta_tmr_res_ms);
    return MTLK_ERR_PARAMS;
  }

  ta->pcoc_cfg.interval = cfg->interval;

  /* Precalculate coefficient for PCoC result conversion */
  ta->pcoc_cfg.coeff =
    8 * (1 << TA_Q_SHIFT) / (ta->pcoc_cfg.interval * TA_PCOC_TICK_MSEC);

  crit = &ta->crit_tbl[TA_CRIT_ID_PCOC];

  /* Convert COC ticks to TA ticks */
  crit->tmr_period = (TA_PCOC_TICK_MSEC * ta->pcoc_cfg.interval / ta_tmr_res_ms);
  ILOG2_D("PCoC timer set to %d ticks", crit->tmr_period);

  /* Restart counters & timer at next TA tick if already running */
  if (crit->fcn){
    crit->tmr_cnt = crit->tmr_period - 1;
    crit->fcn = _ta_crit_pcoc_init;
  }
...
mtlk_ta_on_connect (mtlk_handle_t ta_handle, sta_entry *sta)
{
  ...
  */
  ...
  ILOG3_DY("Create STA WSS entry (0x%08X) for %Y", sta_wss, mtlk_sta_get_addr(sta));
...
mtlk_ta_on_disconnect (mtlk_handle_t ta_handle, sta_entry *sta)
{
  ...
  */
  ...
  ILOG3_DY("Delete STA WSS entry (0x%08X) for %Y", sta_wss, mtlk_sta_get_addr(sta));
...
mtlk_ta_crit_start (mtlk_handle_t crit_handle)
{
  ...
  if (!crit->fcn){
...
#ifdef MTCFG_PMCU_SUPPORT
      case TA_CRIT_ID_PCOC: crit->fcn = _ta_crit_pcoc_init; break;
#endif /*MTCFG_PMCU_SUPPORT*/
      default : break;
    }
    crit->tmr_cnt = crit->tmr_period - 1;
    ...
    ta->active_crit_num ++;
    ILOG2_DD("Criterion %d (0x%08X) STARTED ", (int)crit->id, crit);
  }
...
mtlk_ta_crit_stop (mtlk_handle_t crit_handle)
{
  ...
  if (crit->fcn){
    ta->active_crit_num --;
    crit->fcn = NULL;
    ILOG2_DD("Criterion %d (0x%08X) STOPPED ", (int)crit->id, crit);
  }
...
mtlk_ta_crit_register (mtlk_handle_t ta_handle, ta_crit_id_t crit_id, ta_crit_clb_t clb, mtlk_handle_t clb_ctx)
{
  ...
  ILOG2_DDDD("Callback registered for crit. %d (0x%08X), CLB 0x%08X, CLB_CTX 0x%08X", (int)crit_id, crit, (uint32)clb, (uint32)clb_ctx);
...
mtlk_ta_crit_unregister (mtlk_handle_t crit_handle)
{
  ...
  ILOG2_DD("Unregister criterion %d (0x%08X)", (int)crit->id, crit);
...
mtlk_ta_get_debug_info (mtlk_handle_t ta_handle, struct _mtlk_ta_debug_info_cfg_t *debug_info)
{
  ...
  for (i = 0; i < debug_info->nof_crit; ++i) {
    ...
    crit->fcn        = (uint32)entry->fcn;
    crit->clb        = (uint32)entry->clb;
...
#ifdef MTCFG_PMCU_SUPPORT
    _ta_wss_cntr_t    *pcoc_rx;
    _ta_wss_cntr_t    *pcoc_tx;
#endif /*MTCFG_PMCU_SUPPORT*/
    mtlk_ta_sta_wss_t *wss = &debug_info->sta_wss[i];

    sta_wss = MTLK_LIST_GET_CONTAINING_RECORD(entry, _sta_wss_entry_t, list_entry);
    wss->sta_entry             = (uint32)sta_wss->sta;
...
#ifdef MTCFG_PMCU_SUPPORT
    pcoc_rx = &sta_wss->pcoc_rx_bytes;
    pcoc_tx = &sta_wss->pcoc_tx_bytes;

    wss->pcoc_wss_valid        = sta_wss->pcoc_wss_valid;

    wss->pcoc_rx_bytes.prev    = pcoc_rx->prev;
    wss->pcoc_rx_bytes.delta   = pcoc_rx->delta;

    wss->pcoc_tx_bytes.prev    = pcoc_tx->prev;
    wss->pcoc_tx_bytes.delta   = pcoc_tx->delta;
#endif /*MTCFG_PMCU_SUPPORT*/
