...
{
  MTLK_ASSERT( pqueue != NULL );
  MTLK_ASSERT( vap_mgr != NULL );
  MTLK_ASSERT( pqueue->vap_mgr == NULL );

  pqueue->vap_mgr = vap_mgr;
...
void __MTLK_IFUNC
mtlk_sq_peer_ctx_init(mtlk_sq_t *pqueue, mtlk_sq_peer_ctx_t *ppeer, uint32 limit, mtlk_vap_handle_t vap_handle)
{
    int i;
    ...
    for (i = 0; i < NTS_PRIORITIES; i++)
    {
        ...
        mtlk_osal_atomic_set(&ppeer->limit_per_ac[i], 0);
        mtlk_osal_atomic_set(&ppeer->used_per_ac[i], 0);
    }
    mtlk_osal_atomic_set(&ppeer->ps_mode_enabled, 0);
...
mtlk_sq_peer_ctx_cleanup(mtlk_sq_t *pqueue, mtlk_sq_peer_ctx_t *ppeer)
{
    ...
    for (i = 0; i < NTS_PRIORITIES; i++)
    {
        ...
        while (!mtlk_df_nbuf_list_is_empty(&ppeer->peer_queue_entry[i].buflist))
        {
            ...
            mtlk_df_nbuf_free(pqueue->vap_mgr, ppacket);
        }
                      ...
                      uint16                 access_category)
{
    ...
    do {
      ...
      mtlk_dlist_foreach(&pqueue->peer_queue[access_category].list, entry, head)
      {
        ...
        {
          mtlk_nbuf_t *ppacket;
          uint32 limit_per_ac = mtlk_osal_atomic_get(&ppeer->limit_per_ac[access_category]);

          if (mtlk_osal_atomic_get(&ppeer->ps_mode_enabled) && limit_per_ac) {
            if (mtlk_osal_atomic_get(&ppeer->used_per_ac[access_category]) >= limit_per_ac) {
              continue;
            }
          } else {
          ...
          ppacket = mtlk_df_nbuf_get_by_list_entry(
            ...
            if(__UNLIKELY(pqueue->hw_tx_prohibited)) {
              ...
              mtlk_nbuf_priv_set_tx_pck_ac(mtlk_nbuf_priv(ppacket), access_category);

              res = _mtlk_sq_send_to_hw(ppacket, mtlk_df_nbuf_get_priority(ppacket));
            }
            ...
            case MTLK_ERR_OK:
                ...
                mtlk_osal_atomic_inc(&ppeer->used_per_ac[access_category]);
...
mtlk_sq_set_pm_enabled(mtlk_sq_peer_ctx_t *ppeer, BOOL enabled)
{
  ...
  uint32 limit = enabled
    ? MIN(MTLK_PACKETS_IN_MAC_DURING_PM, ppeer->limit_cfg)
    : ppeer->limit_cfg;

  mtlk_osal_atomic_set(&ppeer->limit, limit);

  mtlk_osal_atomic_set(&ppeer->ps_mode_enabled, enabled);
}
...
mtlk_sq_get_status(mtlk_sq_t *pqueue, mtlk_clpb_t *clpb, BOOL get_peers_status)
{
    ...
    mtlk_dlist_foreach(&pqueue->peer_queue[AC_BE].list, entry, head) {
       ...
       peer_status.vap_handle = ppeer->vap_handle;
       ...
       peer_status.ps_mode_enabled = mtlk_osal_atomic_get(&ppeer->ps_mode_enabled);
       peer_status.limit_uapsd = 0;
       for(idx = 0; idx < NTS_PRIORITIES; idx++) {
         peer_status.uapsd_enabled[idx] = mtlk_osal_atomic_get(&ppeer->limit_per_ac[idx]);
         if (0 == peer_status.limit_uapsd) {
           peer_status.limit_uapsd = peer_status.ps_mode_enabled && peer_status.uapsd_enabled[idx]
             ? mtlk_osal_atomic_get(&ppeer->limit_per_ac[idx])
             : mtlk_osal_atomic_get(&ppeer->limit);
         }
         peer_status.used_per_ac[idx] = mtlk_osal_atomic_get(&ppeer->used_per_ac[idx]);
         ILOG3_DD("### used_per_ac[%u] : %u", idx, peer_status.used_per_ac[idx]);
       }
...
void __MTLK_IFUNC
mtlk_sq_peer_ctx_on_rcvry_restore(mtlk_sq_t *pqueue, mtlk_sq_peer_ctx_t *ppeer, uint32 limit)
{
  int i;
