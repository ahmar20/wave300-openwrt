...
static BOOL _mtlk_is_my_addr(mtlk_core_t *core, IEEE_ADDR *addr);
static BOOL _mtlk_is_from_assoc_ap(mtlk_core_t *core, IEEE_ADDR *bssid);
                       ...
                       mtlk_core_t *core)
{
  ...
  {
    if(coex_sm && mtlk_core_is_20_40_active(core) && !is_ap && mtlk_20_40_check_rssi_threshold(coex_sm, bss_data->max_rssi))
    {
        mtlk_20_40_sta_intolerance_detected(coex_sm, FALSE /* Must acquire the lock */);
    } 
  }
...
static __INLINE int
ie_extract_obss_scan_parameters(uint8 *ie_data,
                                int32 length, 
                                mtlk_20_40_obss_scan_params *received_params)
{
  int res = MTLK_ERR_PARAMS;
  uint16 *u16ie_data = (uint16 *)ie_data;

  if (length != OBSS_SCAN_PARAMETERS_LENGTH){
        ELOG_D("Wrong OBSS Scan Parameters length: %d", (int)length);
  }
  else{
    memset(received_params, 0, sizeof(*received_params));
    
    received_params->uOBSSScanPassiveDwell = WLAN_TO_HOST16 (*u16ie_data);
    u16ie_data++;
    received_params->uOBSSScanActiveDwell = WLAN_TO_HOST16 (*u16ie_data);
    u16ie_data++;
    received_params->uBSSChannelWidthTriggerScanInterval = WLAN_TO_HOST16 (*u16ie_data);
    u16ie_data++;
    received_params->uOBSSScanPassiveTotalPerChannel = WLAN_TO_HOST16 (*u16ie_data);
    u16ie_data++;
    received_params->uOBSSScanActiveTotalPerChannel = WLAN_TO_HOST16 (*u16ie_data);
    u16ie_data++;
    received_params->uBSSWidthChannelTransitionDelayFactor = WLAN_TO_HOST16 (*u16ie_data);
    u16ie_data++;
    received_params->uOBSSScanActivityThreshold = WLAN_TO_HOST16 (*u16ie_data);
...
*/
static void
ie_process (mtlk_handle_t context, uint8 *fbuf, int32 flen, bss_data_t *bss_data, BOOL individually_addressed, BOOL is_beacon, BOOL assoc_res_process, BOOL is_from_assoc_ap)
{
  ...
  while (flen > sizeof(ie_t))
  {
    ...
    if (ie->length) {
        ...
        case IE_COEXISTENCE:
          coex_sm = mtlk_core_get_coex_sm(core);
          if (mtlk_core_is_20_40_active(core))
          {
            mtlk_20_40_coexistence_element coex_el;
                                ...
                                &coex_el) == MTLK_ERR_OK)
            {
              ...
              if(mtlk_20_40_check_rssi_threshold(coex_sm, bss_data->max_rssi))
              {
                ...
                {
                  mtlk_20_40_sta_process_coexistence_element(coex_sm, &coex_el, &bss_addr, individually_addressed, assoc_res_process, is_from_assoc_ap);
                }
              }
            }
          }
        ...
        case IE_OBSS_SCAN_PARAMETERS:
          coex_sm = mtlk_core_get_coex_sm(core);
          if (mtlk_20_40_is_feature_enabled(coex_sm) &&
              (mtlk_core_get_freq_band_cur(core) == MTLK_HW_BAND_2_4_GHZ) &&
              !mtlk_vap_is_ap(core->vap_handle) &&
               (individually_addressed || is_beacon) &&
               is_from_assoc_ap)
          {
            mtlk_20_40_obss_scan_params received_params;
            if (ie_extract_obss_scan_parameters(GET_IE_DATA_PTR(ie),
                                             ie->length,
                                             &received_params) == MTLK_ERR_OK)
            {
              mtlk_20_40_sta_apply_associated_ap_parameters(coex_sm, &received_params);
            }
          }
          break;
        default:
...
_mtlk_process_action_frame_public(mtlk_handle_t    context,
                                 ...
                                 scan_cache_t    *cache,
                                 IEEE_ADDR       *src_addr,
                                 BOOL             individually_addressed,
                                 BOOL             is_beacon,
                                 BOOL             is_from_assoc_ap)
{
  uint8 action = *(uint8 *)fbuf;
  bss_data_t *bss_data;
  ...
  switch (action)
  {
    case ACTION_FRAME_ACTION_PUBLIC_COEXISTENCE:
        /* 20/40 action Frame contains coexistence element and zero or more shows of intolerant channel report */
        ...
        ie_process(context, fbuf, (int32)flen, bss_data, individually_addressed, is_beacon, FALSE, is_from_assoc_ap);
                                       ...
                                       BOOL             is_from_assoc_ap)
{
  ...
  /* We're going to put it there */
  mtlk_osal_copy_eth_addresses(bss_data->bssid, src_addr->au8Addr);
  ie_process(context, fbuf, (int32)flen, bss_data, individually_addressed, is_beacon, assoc_res_process, is_from_assoc_ap);
  mtlk_cache_temp_bss_release(cache);
}
...
mtlk_process_man_frame (mtlk_handle_t context, sta_entry *sta, struct mtlk_scan *scan_data, scan_cache_t* cache, 
    mtlk_aocs_t *aocs, mtlk_nbuf_t *nbuf, const MAC_RX_ADDITIONAL_INFO_T *mac_rx_info)
{
  int res = MTLK_ERR_OK;
  uint8 *fbuf = nbuf->data;
  int32 flen = nbuf->len;
  mtlk_core_t *core = ((mtlk_core_t*)context);
  uint16 frame_ctrl;
  uint16 subtype;
  uint8 protected_frame;
  frame_head_t *head;
  ...
  protected_frame = !!WLAN_FC_GET_PROTECTED(frame_ctrl);

  ILOG4_D("Subtype is %d", subtype);

  if(protected_frame)
  {
    int replay_detection_result = 0;

    if(sta)
    {
      (void) get_rsc_buf(core, nbuf, sizeof(frame_head_t));
      replay_detection_result = detect_replay(core, nbuf, sta->pmf_rsc, TRUE);
    }
    else
    {
      WLOG_V("Protected Management Frame received from unknown STA");
      res = MTLK_ERR_NOT_IN_USE;
      goto end;
    }

    if(replay_detection_result)
    {
      res = MTLK_ERR_NOT_IN_USE;
      goto end;
    }

    fbuf += IEEE80211_CCMP_HEADER_LEN;
  ...
  case MAN_TYPE_ASSOC_REQ:
    ...
    if (mtlk_20_40_is_feature_enabled(mtlk_core_get_coex_sm(core)) &&
       (mtlk_core_get_freq_band_cfg(core) == MTLK_HW_BAND_2_4_GHZ))
    {
      ILOG3_V("MAN_TYPE_ASSOC_REQ arrived. Parsing...");
      _mtlk_process_coex_info_from_man_frame(context, subtype, fbuf, flen, cache, (IEEE_ADDR *)head->src_addr, _mtlk_is_my_addr(core, (IEEE_ADDR *)head->dst_addr), is_beacon, assoc_res_process, _mtlk_is_from_assoc_ap(core, (IEEE_ADDR *)head->bssid));
    }
    break;
  case MAN_TYPE_ASSOC_RES:
    CPU_STAT_SPECIFY_TRACK(CPU_STAT_ID_RX_MGMT_OTHER);
    assoc_res_process = TRUE;
    if (mtlk_20_40_is_feature_enabled(mtlk_core_get_coex_sm(core)) &&
       (mtlk_core_get_freq_band_cfg(core) == MTLK_HW_BAND_2_4_GHZ))
    {
  ...
  case MAN_TYPE_PROBE_RES:
    ...
    ie_process(context, fbuf, (int32)flen, bss_data, _mtlk_is_my_addr(core, (IEEE_ADDR *)head->dst_addr), is_beacon, assoc_res_process, _mtlk_is_from_assoc_ap(core, (IEEE_ADDR *)head->bssid));
    ...
    if (mtlk_core_scan_is_running(core) || !mtlk_core_get_country_code(core) || mtlk_20_40_is_feature_enabled(coex_sm)) {
      ...
      if (
          ...
          mtlk_vap_is_ap(core->vap_handle) &&
          mtlk_core_get_freq_band_cfg(core) == MTLK_HW_BAND_2_4_GHZ &&
          mtlk_20_40_check_rssi_threshold(coex_sm, bss_data->max_rssi)
      ) {
    ...
    /* WDS beacon processing */
    /* Assuming beacon arrived on Master VAP only */
    if (mtlk_vap_is_ap(core->vap_handle) && (MAN_TYPE_BEACON == subtype)){ 
        mtlk_core_wds_on_beacon(core, (IEEE_ADDR *)head->src_addr);
    } 
    ...
    CPU_STAT_SPECIFY_TRACK(CPU_STAT_ID_RX_MGMT_ACTION);
    {
      ...
      case ACTION_FRAME_CATEGORY_PUBLIC:
        _mtlk_process_action_frame_public(context,
                                          ...
                                          cache,
                                          (IEEE_ADDR *)head->src_addr,
                                          _mtlk_is_my_addr(core, (IEEE_ADDR *)head->dst_addr),
                                          is_beacon, 
                                          _mtlk_is_from_assoc_ap(core, (IEEE_ADDR *)head->bssid));
        break;
...
static BOOL _mtlk_is_my_addr(mtlk_core_t *core, IEEE_ADDR *addr)
{
  ...
  uint8 my_addr[ETH_ALEN];

  MTLK_ASSERT(NULL != core);

  mtlk_pdb_get_mac(mtlk_vap_get_param_db(core->vap_handle), PARAM_DB_CORE_MAC_ADDR, my_addr);
  if (!memcmp(addr, my_addr, sizeof(*addr)))
  {
    is_my_addr = TRUE;
  }
...
static BOOL _mtlk_is_from_assoc_ap(mtlk_core_t *core, IEEE_ADDR *bssid)
{
  ...
  if (!mtlk_vap_is_ap(core->vap_handle))
  {
    sta = mtlk_stadb_get_ap(&core->slow_ctx->stadb);
    if(sta)
    {
      if (memcmp(bssid->au8Addr, mtlk_sta_get_addr(sta)->au8Addr, IEEE_ADDR_LEN) == 0) /* ASSOC. AP */
      {
        res = TRUE;
      }
      mtlk_sta_decref(sta);
