...
static uint16 _slave_vap_specific_mibs[] = {
  MIB_ACL_MODE,
...
mtlk_mib_request_is_allowed (mtlk_vap_handle_t vap_handle, uint16 u16mib_id)
{
  ...
  if (!mtlk_vap_is_master(vap_handle)) {
    scIndex = ARRAY_SIZE(_slave_vap_specific_mibs) - 1;
    while ((scIndex >= 0) && (_slave_vap_specific_mibs[scIndex] != u16mib_id)) {
      scIndex --;
    }
...
aux_pm_related_set_to_mac_blocked (mtlk_txmm_t                  *txmm,
                                   ...
                                   uint32                        supported_rate_set,
                                   uint32                        extended_rate_set,
                                   uint8                         upper_lower,
                                   uint8                         spectrum,
                                   mtlk_aux_pm_related_params_t *result)
{
  ...
  result->u32BSSOperationalRateSet = operational_rate_set;
  result->u32BSSbasicRateSet    = basic_rate_set;
  result->u32BSSSupportedRateSet = supported_rate_set;
  result->u32BSSExtendedRateSet = extended_rate_set;
  ...
  if (!is_ht_net_mode(net_mode)) {
    result->u32BSSOperationalRateSet &= LM_PHY_11A_RATE_MSK | LM_PHY_11B_RATE_MSK;
  ...
  if (net_mode_to_band(net_mode) == MTLK_HW_BAND_2_4_GHZ) {
    if (result->u32BSSOperationalRateSet & (~LM_PHY_11B_RATE_MSK)) {
  ...
  data->u32BSSOperationalRateSet         = HOST_TO_MAC32(result->u32BSSOperationalRateSet);
  data->u32BSSbasicRateSet               = HOST_TO_MAC32(result->u32BSSbasicRateSet);
  data->u32BSSSupportedRateSet           = HOST_TO_MAC32(result->u32BSSSupportedRateSet);
  data->u32BSSExtendedRateSet            = HOST_TO_MAC32(result->u32BSSExtendedRateSet);
  ...
  ILOG2_DDDDDDDDD("SENDING PREACTIVATION MIB:\n"
       ...
       "result->u32BSSOperationalRateSet            = %X\n"
       "result->u32BSSbasicRateSet                  = %X\n"
       "result->u32BSSSupportedRateSet              = %X\n"
       "result->u32BSSExtendedRateSet               = %X",
      ...
      result->u32BSSOperationalRateSet,
      result->u32BSSbasicRateSet,
      result->u32BSSSupportedRateSet,
      result->u32BSSExtendedRateSet);
...
mtlk_aux_pm_related_params_set_defaults (mtlk_txmm_t                  *txmm,
                                         ...
                                         mtlk_aux_pm_related_params_t *result)
{
  mtlk_core_t *nic = mtlk_vap_get_core(txmm->vap_handle);

  return aux_pm_related_set_to_mac_blocked(txmm,
           net_mode,
           get_operate_rate_set(net_mode),
           user_rates_to_fw(nic, get_basic_rateset(nic, net_mode, TRUE)),
           user_rates_to_fw(nic, get_supported_rateset(nic, net_mode, TRUE)),
           user_rates_to_fw(nic, get_extended_rateset(nic, net_mode, TRUE)),
           ALTERNATE_UPPER,
           spectrum,
           result);
}
                                         ...
                                         mtlk_aux_pm_related_params_t *result)
{
  ...
  MTLK_ASSERT(!mtlk_osal_is_zero_address(bss_data->bssid));

  return aux_pm_related_set_to_mac_blocked(txmm,
                                           net_mode,
                                           bss_data->operational_rate_set,
                                           bss_data->basic_rate_set,
                                           bss_data->supported_rate_set,
                                           bss_data->extended_rate_set,
...
mtlk_set_mib_pre_activate (mtlk_txmm_t *txmm, mtlk_aux_pm_related_params_t *params)
{
  ...
  data->u32BSSOperationalRateSet         = HOST_TO_MAC32(params->u32BSSOperationalRateSet);
  data->u32BSSbasicRateSet               = HOST_TO_MAC32(params->u32BSSbasicRateSet);
  data->u32BSSSupportedRateSet           = HOST_TO_MAC32(params->u32BSSSupportedRateSet);
  data->u32BSSExtendedRateSet            = HOST_TO_MAC32(params->u32BSSExtendedRateSet);
  ...
  ILOG2_DDDDDDDDD("SENDING PREACTIVATION MIB:\n"
      ...
      data->u32BSSOperationalRateSet,
      data->u32BSSbasicRateSet,
      data->u32BSSSupportedRateSet,
      data->u32BSSExtendedRateSet);
