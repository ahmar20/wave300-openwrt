...
#define MTLK_SQ_TX_LIMIT_LEGACY    2
#define MTLK_PACKETS_IN_MAC_DURING_PM 3
/* U-APSD */
#define UAPSD_MAX_SP_ADDITION  3
#define UAPSD_MAX_SP_USER_DEFAULT 4
#define UAPSD_MAX_SP_USER_MIN 2
#define UAPSD_MAX_SP_USER_MAX 6
    uint32            limit_cfg;
    mtlk_atomic_t     limit; /* maximum allowed MSDU amount which STA can occupy */
    mtlk_atomic_t     used;
    mtlk_sq_stats_t   stats;
    mtlk_vap_handle_t vap_handle;
    mtlk_atomic_t     ps_mode_enabled;
    mtlk_atomic_t     limit_per_ac[NTS_PRIORITIES];
    mtlk_atomic_t     used_per_ac[NTS_PRIORITIES];
    uint8             uapsd_max_sp;
} mtlk_sq_peer_ctx_t;
...
typedef struct _mtlk_sq_t
{
    ...
    mtlk_vap_manager_t*  vap_mgr;                                 //VAP manager object
...
typedef struct _mtlk_sq_peer_status
{
  ...
  mtlk_vap_handle_t vap_handle;    /* VAP handle for broadcast queue */
...
\param   pqueue  Pointer to user-allocated memory for send queue structure
\param   vap_mgr Pointer to VAP manager object
*/
...
/*!
\fn      void __MTLK_IFUNC mtlk_sq_peer_ctx_init(mtlk_sq_t *pqueue, mtlk_sq_peer_ctx_t *ppeer, uint32 resource, mtlk_vap_handle_t vap_handle)
...
\param   VAP handle for VAP (NULL for STA)
*/
void __MTLK_IFUNC
mtlk_sq_peer_ctx_on_rcvry_restore(mtlk_sq_t *pqueue, mtlk_sq_peer_ctx_t *ppeer, uint32 limit);

void __MTLK_IFUNC
mtlk_sq_peer_ctx_init(mtlk_sq_t *pqueue, mtlk_sq_peer_ctx_t *ppeer, uint32 resource, mtlk_vap_handle_t vap_handle);
...
/*!
=fn      void __MTLK_IFUNC mtlk_sq_on_tx_cfm(mtlk_sq_peer_ctx_t *ppeer)
\brief   Request to free TX resouce for ppeer

\param   ppeer SendQueue peer ctx
\param   ac    Access category of confirmed packet
*/
static void __INLINE
mtlk_sq_on_tx_cfm(mtlk_sq_peer_ctx_t *ppeer, uint8 ac )
{
    ASSERT( ppeer != NULL );
    ASSERT( ac < NTS_PRIORITIES );

    mtlk_osal_atomic_dec(&ppeer->used);
    mtlk_osal_atomic_dec(&ppeer->used_per_ac[ac]);
}
...
/*!
=fn      int __MTLK_IFUNC mtlk_sq_wait_all_packets_confirmed(mtlk_sq_peer_ctx_t *ppeer)
...
{
#define SQ_WAIT_ALL_PACKETS_CFM_TIMEOUT_TOTAL  (3*30*MSEC_PER_SEC)
#define SQ_WAIT_ALL_PACKETS_CFM_TIMEOUT_ONCE   10
  ...
  return res;
#undef SQ_WAIT_ALL_PACKETS_CFM_TIMEOUT_ONCE
#undef SQ_WAIT_ALL_PACKETS_CFM_TIMEOUT_TOTAL
}

int mtlk_sq_enqueue_clone_begin (mtlk_sq_t *sq, uint16 ac, BOOL front, mtlk_nbuf_t *nbuf);
int mtlk_sq_enqueue_clone(struct nic *nic, mtlk_nbuf_t *nbuf, sta_entry *clone_dst_sta);
...
int mtlk_sq_enqueue(struct nic *nic, uint16 ac, BOOL front, struct sk_buff *skb);
