...
mtlk_peer_analyzer_init(mtlk_peer_analyzer_t *peer_analyzer)
{
  ...
  for (i = 0; i < NUMBER_OF_RX_ANTENNAS; ++i) {
    ...
    _mtlk_count_averager_init(&peer_analyzer->short_rssi_average[i], _MTLK_PEER_SHORT_TERM_RSSI_PACKETS);
  }
...
void __MTLK_IFUNC
mtlk_peer_analyzer_process_rssi_sample(mtlk_peer_analyzer_t *peer_analyzer, const uint8 rssi_samples[])
{
  int i;
  mtlk_osal_lock_acquire(&peer_analyzer->analyzer_lock);

  for (i = 0; i < NUMBER_OF_RX_ANTENNAS; ++i) {
    _mtlk_count_averager_process_sample(&peer_analyzer->short_rssi_average[i], rssi_samples[i]);
#if MTLK_WWSS_WLAN_STAT_GROUP_ANALYZER_RX_LONG_ALLOWED
...
uint32 __MTLK_IFUNC
mtlk_peer_analyzer_get_short_term_rssi(const mtlk_peer_analyzer_t *peer_analyzer, int rssi_index)
{
  uint32 res;

  mtlk_osal_lock_acquire((mtlk_osal_spinlock_t*) &peer_analyzer->analyzer_lock);
  res = _mtlk_count_averager_get_value(&peer_analyzer->short_rssi_average[rssi_index]);
  mtlk_osal_lock_release((mtlk_osal_spinlock_t*) &peer_analyzer->analyzer_lock);
  return res;
}
