...
/* restrict channels mode */
enum {
    AOCS_RESTRICT_CHAN_MODE_PRIMARY = 0,
    AOCS_RESTRICT_CHAN_MODE_ALL,
    AOCS_RESTRICT_CHAN_MODE_LAST
};
...
typedef struct _mtlk_aocs_channels_stat_t {
  ...
  uint16 nof_bss;
  uint8  channel_load;
  uint16 num_20mhz_bss;
  uint16 num_40mhz_up_bss;
  uint16 num_40mhz_lw_bss;
...
typedef struct {
  ...
  int8 MaxMetric;
  int8 MinMetric;
  int8 selected_threshold;
} __MTLK_IDATA mtlk_aocs_interfered_stat_entry_t;
...
typedef enum _mtlk_aocs_event_e {
  ...
  MTLK_AOCS_EVENT_INTERF_IND,
  /* interference detection statistics */
  MTLK_AOCS_EVENT_INTERF_STAT,
...
int __MTLK_IFUNC mtlk_aocs_get_interfered(mtlk_aocs_t *aocs, mtlk_clpb_t *clpb);
...
void __MTLK_IFUNC mtlk_aocs_on_rcvry_isol(mtlk_aocs_t *aocs);
int __MTLK_IFUNC mtlk_aocs_on_rcvry_restore(mtlk_aocs_t *aocs);
...
#ifdef BT_ACS_DEBUG
void mtlk_aocs_set_channels_dbg(mtlk_aocs_t *aocs, mtlk_aocs_channel_data_t *entry);
mtlk_slist_t* get_aocs_channel_list(mtlk_aocs_t *aocs);
#endif  /* BT_ACS_DEBUG */
...
int __MTLK_IFUNC mtlk_aocs_set_non_wifi_interf_is_enabled(mtlk_aocs_t *aocs, BOOL value);
int __MTLK_IFUNC mtlk_aocs_set_non_wifi_ch_load_trh(mtlk_aocs_t *aocs, uint8 value);
int __MTLK_IFUNC mtlk_aocs_set_non_wifi_RSSI_trh(mtlk_aocs_t *aocs, int8 value);
struct _mtlk_interf_t;
void __MTLK_IFUNC mtlk_aocs_get_non_wifi_interf(mtlk_aocs_t *aocs, struct _mtlk_interf_t *non_wifi_interf);
...
BOOL __MTLK_IFUNC mtlk_aocs_is_unrestricted_available(mtlk_aocs_t *aocs, uint8 *exclude_chans);
BOOL __MTLK_IFUNC mtlk_aocs_is_restricted_chnl(mtlk_aocs_t *aocs, uint16 channel);
...
void __MTLK_IFUNC
mtlk_aocs_set_bonding(mtlk_aocs_t *aocs, uint8 bonding);

uint8 __MTLK_IFUNC
mtlk_aocs_get_bonding(mtlk_aocs_t *aocs);
...
void __MTLK_IFUNC
mtlk_aocs_set_restrict_mode(mtlk_aocs_t *aocs, uint8 restrict_mode);

uint8 __MTLK_IFUNC
mtlk_aocs_get_restrict_mode(mtlk_aocs_t *aocs);
...
mtlk_aocs_channel_data_t **__MTLK_IFUNC
mtlk_aocs_create_chann_arr_asc(mtlk_aocs_t *aocs);

uint32 __MTLK_IFUNC
mtlk_aocs_chan_list_size(mtlk_aocs_t *aocs);

void __MTLK_IFUNC
mtlk_aocs_scan_floor_noise_ind(mtlk_aocs_t *aocs, UMI_INTERFERER *umi_interf_ind);

BOOL __MTLK_IFUNC
mtlk_aocs_find_secondary_channel_no(mtlk_aocs_t *aocs, uint16 primary, uint16 secondary);
