...
static __INLINE mtlk_nbuf_t *
__mtlk_df_nbuf_alloc (mtlk_vap_manager_t *vap_mgr, uint32 size, mtlk_slid_t caller_slid)
{
  mtlk_nbuf_t * nbuf = _mtlk_df_nbuf_alloc_osdep(vap_mgr, size);
  mtlk_objpool_add_nbuf(nbuf, caller_slid);
  return nbuf;
}

#define mtlk_df_nbuf_alloc(vm, size) __mtlk_df_nbuf_alloc((vm), (size), MTLK_SLID)

static __INLINE void mtlk_df_nbuf_free(mtlk_vap_manager_t *vap_mgr, mtlk_nbuf_t *nbuf)
{
  mtlk_objpool_remove_nbuf(nbuf);
  _mtlk_df_nbuf_free_osdep(vap_mgr, nbuf);
}

/* free skb buffer with all subframes for a-msdu */
static __INLINE void mtlk_df_nbuf_free_sub_frames(mtlk_vap_manager_t *vap_mgr, mtlk_nbuf_t *nbuf)
{
  ...
  do
  {
    ...
    mtlk_df_nbuf_free(vap_mgr, nbuf);
    nbuf = nbuf_next;
  } while(nbuf);
}

static __INLINE mtlk_nbuf_t *
__mtlk_df_nbuf_clone_no_priv (mtlk_vap_manager_t *vap_mgr, mtlk_nbuf_t *nbuf, mtlk_slid_t caller_slid)
{
  mtlk_nbuf_t *nbuf_new = _mtlk_df_nbuf_clone_no_priv_osdep(vap_mgr, nbuf);
  mtlk_objpool_add_nbuf(nbuf_new, caller_slid);
  return nbuf_new;
}

#define mtlk_df_nbuf_clone_no_priv(vm, nbuf) __mtlk_df_nbuf_clone_no_priv((vm), (nbuf), MTLK_SLID)

static __INLINE mtlk_nbuf_t *
__mtlk_df_nbuf_clone_with_priv (mtlk_vap_manager_t *vap_mgr, mtlk_nbuf_t *nbuf, mtlk_slid_t caller_slid)
{
  mtlk_nbuf_t *nbuf_new = _mtlk_df_nbuf_clone_with_priv_osdep(vap_mgr, nbuf);
  mtlk_objpool_add_nbuf(nbuf_new, caller_slid);
  return nbuf_new;
}

#define mtlk_df_nbuf_clone_with_priv(vm, nbuf) __mtlk_df_nbuf_clone_with_priv((vm), (nbuf), MTLK_SLID)
