...
mtlk_cache_register_bss (scan_cache_t *cache, bss_data_t *bss)
{
  ...
  if (slot == NULL) {
    ...
    if (slot == NULL) {
      ILOG3_Y("Failed to register BSS %Y", bss->bssid);
      goto out;
    }
  ...
  ILOG3_Y("BSS %Y registered/updated", slot->bss.bssid);
...
void __MTLK_IFUNC
mtlk_cache_clear_except_entry (scan_cache_t *cache, const uint8 *bssid)
{
    BOOL found = FALSE;
    mtlk_slist_entry_t *list_entry_to_delete = NULL;
    ...
    while ((list_entry_to_delete = mtlk_slist_pop(&cache->bss_list)) != NULL)
    {
        ...
        if (!memcmp(cache_entry_to_delete->bss.bssid, bssid, IEEE_ADDR_LEN))
        {
            found = TRUE;
...
uint32 __MTLK_IFUNC mtlk_cache_modify_expiration_time(scan_cache_t *cache, unsigned long cache_expire, BOOL is_modification_allowed)
{
  int res = MTLK_ERR_PROHIB;
  
  if (is_modification_allowed)
  {
    mtlk_cache_set_expiration_time(cache, cache_expire);
    res = MTLK_ERR_OK;
  }
