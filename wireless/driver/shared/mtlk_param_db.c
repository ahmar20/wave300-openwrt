...
static int _mtlk_pdb_init_values_set(mtlk_pdb_t *obj, mtlk_pdb_initial_value const **initial_values)
{
  ...
  {
    for (value = (*initial_values); (NULL != value) && (value->id != PARAM_DB_LAST_VALUE_ID); value++)
    {
      MTLK_ASSERT(value->id < PARAM_DB_LAST_VALUE_ID);
      ...
      switch(value->type) {
        ...
        case PARAM_DB_TYPE_STRING:
            ...
            strncpy(obj->storage[value->id].value.value_ptr,
                    (char*)value->value,
                    (value->size - 1));
        ...
        case PARAM_DB_TYPE_MAC:
            ...
            memcpy(obj->storage[value->id].value.value_ptr,
                   value->value,
                   value->size);
...
static int _mtlk_pdb_fill_stored_params (mtlk_pdb_t * obj, mtlk_card_type_t hw_type) {
  int i;
  ...
  result = _mtlk_pdb_init_values_set(obj, mtlk_pdb_initial_values);
  if (MTLK_ERR_OK == result)
  {
    /* do HW depended initialization */
    CARD_SELECTOR_START(hw_type)
    IF_CARD_PCIE  ( result = _mtlk_pdb_init_values_set(obj, mtlk_pdb_initial_values_pcie);  );
    IF_CARD_PCIG3 ( result = _mtlk_pdb_init_values_set(obj, mtlk_pdb_initial_values_pcig3); );
    IF_CARD_AHBG35( result = _mtlk_pdb_init_values_set(obj, mtlk_pdb_initial_values_g35);   );
    CARD_SELECTOR_END();
  }
...
_mtlk_pdb_init(mtlk_pdb_t* obj, mtlk_card_type_t hw_type) {
  ILOG4_V(">>");
  ...
  MTLK_INIT_TRY(param_db, MTLK_OBJ_PTR(obj))
    ...
    MTLK_INIT_STEP(param_db, PARAM_DB_SET_UNINITIALIZED, MTLK_OBJ_PTR(obj),
                   _mtlk_pdb_fill_stored_params, (obj, hw_type));
...
mtlk_pdb_create(mtlk_card_type_t hw_type) {
  mtlk_pdb_t *param_db;
  ILOG4_V(">>");
  ...
  if (MTLK_ERR_OK != _mtlk_pdb_init(param_db, hw_type)) {
