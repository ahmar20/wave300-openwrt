...
static const mtlk_ability_id_t _scan_common_abilities[] = {
  MTLK_CORE_REQ_GET_SCANNING_RES
};

static int
_mtlk_scan_sta_abilities_init(struct mtlk_scan *scan)
{
  int res = MTLK_ERR_OK;
...
static void
_mtlk_scan_sta_abilities_cleanup(struct mtlk_scan *scan)
{
  if (!mtlk_vap_is_ap(scan->vap_handle)) {
...
static int
_mtlk_scan_common_abilities_init(struct mtlk_scan *scan)
{
  int res = MTLK_ERR_OK;
  res = mtlk_abmgr_register_ability_set(mtlk_vap_get_abmgr(scan->vap_handle),
                                        _scan_common_abilities, ARRAY_SIZE(_scan_common_abilities));
  if (MTLK_ERR_OK == res) {
      mtlk_abmgr_enable_ability_set(mtlk_vap_get_abmgr(scan->vap_handle),
                                    _scan_common_abilities, ARRAY_SIZE(_scan_common_abilities));
  }
...
static void
_mtlk_scan_common_abilities_cleanup(struct mtlk_scan *scan)
{
  mtlk_abmgr_disable_ability_set(mtlk_vap_get_abmgr(scan->vap_handle),
                                 _scan_common_abilities, ARRAY_SIZE(_scan_common_abilities));
  mtlk_abmgr_unregister_ability_set(mtlk_vap_get_abmgr(scan->vap_handle),
...
switch_back_progmodel(struct mtlk_scan *scan)
{
  ...
  /* BSS data from scan config */
  bss_data = scan->config.bss_data;
...
init_default_params(struct mtlk_scan_params *params)
{
  ...
  params->is_obss = FALSE;
}

static void
init_default_interf_params (struct mtlk_scan_interf_params *interf_params)
{
  memset(interf_params, 0, sizeof(struct mtlk_scan_interf_params));

//  interf_params->max_scan_time = 1000;
}
...
mtlk_scan_init (struct mtlk_scan *scan, struct mtlk_scan_config config, mtlk_vap_handle_t vap_handle)
{
  ...
  scan->scan_completed_notification_callback = NULL;
  scan->scan_completed_notification_context = HANDLE_T(NULL);
  scan->vap_handle = vap_handle;
  scan->config = config;
  init_default_params(&scan->params);
  init_default_interf_params(&scan->interf_params);
  ...
  scan->must_load_progmodels = TRUE;
  ...
  res = _mtlk_scan_sta_abilities_init(scan);
  if (MTLK_ERR_OK != res) {
    goto err_alloc_last_result;
  }

  res = _mtlk_scan_common_abilities_init(scan);
  if (MTLK_ERR_OK != res) {
    goto err_sta_abilities;
  }
...
err_sta_abilities:
  _mtlk_scan_sta_abilities_cleanup(scan);
...
mtlk_scan_cleanup (struct mtlk_scan *scan)
{
  ...
  _mtlk_scan_common_abilities_cleanup(scan);
...
complete_or_continue(struct mtlk_scan *scan)
{
  if (scan->next_band == MTLK_HW_BAND_NONE) {
    ...
    if (scan->scan_completed_notification_callback)
    {
      (*scan->scan_completed_notification_callback)(scan->scan_completed_notification_context);
    }
...
ERROR:
  delete_progmodels(scan);
  // Here we can assume the current policy is to load progmodels during scan - otherwise we wouldn't have gotten to this function
...
do_scan(struct mtlk_scan *scan, uint8 band, uint8 spectrum, const char* essid, BOOL is_sync)
{
  ...
  if(scan->params.is_background) {
    ...
    MTLK_ASSERT(!scan->interf_params.is_interf_enabled);
  } else {
    ILOG0_V("Starting normal scan...");
  }

  /* Clear cached scan results */
  if (scan->interf_params.is_interf_enabled) {
    /* don't clear cache */;
  }
  else {
...
static void
scan_store_calibration_channel_bit_map (struct mtlk_scan *scan, const void *payload)
{
  mtlk_core_store_calibration_channel_bit_map(mtlk_vap_get_core(scan->vap_handle),
                                              ((UMI_SCAN *)payload)->storedCalibrationChannelBitMap);
}
...
mtlk_scan_handle_evt_scan_confirmed (mtlk_handle_t scan_object, const void *payload, uint32 size)
{
  ...
  if (mtlk_vap_is_master_ap(scan->vap_handle)) {
    ...
    scan_store_calibration_channel_bit_map(scan, payload);
  }

  if (scan->params.is_background && scan->params.pause_between_chunks)
    /* Defer scanning to user-configured time.
     * This is needed to not to affect traffic, that is currently passing through.
     */
...
{
  MTLK_ASSERT(NULL != scan);
  MTLK_ASSERT(mtlk_vap_is_master_ap(scan->vap_handle));

  return scan->interf_params.is_interf_enabled;
}
...
mtlk_scan_set_is_interf_enabled (struct mtlk_scan *scan, BOOL is_interf_enabled)
{
  ...
  scan->interf_params.is_interf_enabled = is_interf_enabled;
}
...
mtlk_scan_set_is_long_scan_enabled (struct mtlk_scan *scan, BOOL is_long_scan_enabled)
{
  ...
  scan->interf_params.is_long_scan_enabled = is_long_scan_enabled;
}
void __MTLK_IFUNC
mtlk_scan_set_scan_time (struct mtlk_scan *scan, uint16 long_scan_time, uint16 short_scan_time)
{
  ...
  scan->interf_params.max_long_scan_time  = long_scan_time;
  scan->interf_params.max_short_scan_time = short_scan_time;
}

//void __MTLK_IFUNC
//mtlk_scan_set_is_long_scan_active (struct mtlk_scan *scan, BOOL is_long_scan_active)
//{
//  MTLK_ASSERT(NULL != scan);
//  MTLK_ASSERT(mtlk_vap_is_master_ap(scan->vap_handle));
//  MTLK_ASSERT(scan->interf_params.is_interf_enabled);
//  MTLK_ASSERT(scan->interf_params.is_long_scan_enabled);
//
//  scan->interf_params.is_long_scan_active = is_long_scan_active;
//}
...
void __MTLK_IFUNC
mtlk_scan_set_obss(struct mtlk_scan *scan, BOOL is_obss)
{
  scan->params.is_obss = is_obss;
}
...
void __MTLK_IFUNC
mtlk_scan_register_scan_completed_notification_callback(struct mtlk_scan *scan, mtlk_handle_t context, scan_completed_notification_callback_type *callback)
{
  scan->scan_completed_notification_callback = callback;
  scan->scan_completed_notification_context = context;
}
...
mtlk_scan_set_load_progmodel_policy(struct mtlk_scan *scan, BOOL must_load)
{
  if(!mtlk_scan_is_running(scan))
  {
    scan->must_load_progmodels = must_load;
  }
}
