...
{
  uint32 const DUT_MSG_HEADER_LENGTH = 8;
  ...
  ILOG0_DS("DUT%d: %s", hw_idx, __FUNCTION__);
...
_read_cmp (int hw_idx, int storage)
{
  ...
  ILOG0_DSD("DUT%d: %s (%d)", hw_idx, __FUNCTION__, acc_count);
...
_rw_error (int hw_idx, int storage)
{
  ...
  ILOG0_DSD("DUT%d: %s (%d)", hw_idx, __FUNCTION__, offset_count);
