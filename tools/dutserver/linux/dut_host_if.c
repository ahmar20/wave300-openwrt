...
{
    uint32 raw_length;
    memcpy(&raw_length, hdr + sizeof(uint32), sizeof(uint32));
...
_dut_hostif_send_response(int msgID, const char* data, int dataLength)
{
  ...
  ILOG0_DPD("Sending response msgID 0x%X, data 0x%p, length %d",
...
dut_hostif_handle_packet (const char* inBuf, int packetLength)
{
  ...
  {
    ELOG_V("Wrong packet received. MT signature not found");
    return;
  }
  ...
  ILOG0_DDD("Received message ID 0x%X for HW #%d, length is %d",
            msgID, dutIndex, DUT_MSG_HEADER_LENGTH + dataLength);

  msgHandler = dut_msg_clbk_get_handler(msgID);
  if(NULL == msgHandler)
  {
    ELOG_D("Incorrect message ID 0x%X", dutIndex);
    return;
  }
  ...
  {
    msgHandler->pOnDutRequest(inBuf+8, dataLength, dutIndex);
    ILOG0_V("Message handled");
  }
  else
  {
