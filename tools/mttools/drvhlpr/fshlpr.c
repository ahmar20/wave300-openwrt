...
#define EEPROM_PUT8(v) { (*p) = (v); p++; }
  ...
  p_crc = _eeprom_cis_find(EEPROM_CIS_CRC_ID,
  ...
  if(p_crc)
  {
    /*CIS CRC already persist*/
    /* Do CRC calculation for first chunk before CIS CRC*/
    int chunk_size;

    chunk_size = p_crc - src + CIS_SECTION_HEADER_SIZE;
    crc = _crc32(~0L, dst, chunk_size);
    ILOG1_DD("FS HLPR: Calculate CRC32 [0x%08x] for EEPROM data first chunk [%d]", crc, chunk_size);

    /* Do CRC calculation for second chunk after CIS CRC*/
    chunk_size = (p - dst) - (p_crc - src) - EEPROM_CRC_SECTION_SIZE;
    crc = _crc32(crc, dst + (p_crc - src) + EEPROM_CRC_SECTION_SIZE, chunk_size);
    ILOG1_DD("FS HLPR: Calculate CRC32 [0x%08x] for EEPROM data second chunk [%d]", crc, chunk_size);
    crc ^= ~0L;
    ILOG1_DD("FS HLPR: Calculate CRC32 [0x%08x] for EEPROM data second chunk [%d]", crc, chunk_size);

    /*update pointer to CIS CRC area*/
    p = dst + (p_crc - src) + CIS_SECTION_HEADER_SIZE;
  }
  else
  {
    /* write CIS CRC*/
    ...
    crc ^= ~0L;
    ILOG1_DD("FS HLPR: Calculate CRC32 [0x%08x] for EEPROM data [%d]", crc, p - dst);
  }
