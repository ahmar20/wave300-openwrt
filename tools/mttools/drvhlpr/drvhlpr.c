...
static char recovery_script_path[MTLK_RECOVERY_STRING_BUFFER_SIZE] = "";
...
{
  ILOG1_D("set event %d", status);
  close_status = status;
  mtlk_osal_event_set(&close_evt);
}

static int
drvhlpr_set_recovery_script_path (char* str)
{
  int size = strlen(str) + 1;
  if (size <= sizeof(recovery_script_path)) {
    memcpy(recovery_script_path, str, size);
    ILOG2_S("recovery_script_path = %s", recovery_script_path);
    return MTLK_ERR_OK;
  }
...
load_cfg (const char *cfg_fname)
{
  ...
  if (tmp) {
    res = drvhlpr_set_recovery_script_path(tmp);
    if (res != MTLK_ERR_OK) {
...
_drvhlpr_run_script (const char *command)
{
  ...
  /* do not run script if mask is not set */
                  ...
                  uint32            size)
{
  if (mtlk_guid_compare(evt, &IRBE_HANG) == 0) {
    ...
    /* run script for the specified interface only */
    if (!strncmp((char *) buffer, wlanitrf, IFNAMSIZ)) {
      _drvhlpr_run_script(_DRVHLPR_SCRIPT_CMD_STOP);
    }

    /* do RESET for any interface */
    ...
    _drvhlpr_run_script(_DRVHLPR_SCRIPT_CMD_START);
    ...
    _drvhlpr_run_script(_DRVHLPR_SCRIPT_CMD_STOP);
    initiate_app_shutdown(EVENT_REQ_RESTART);
  }
  else {
...
main (int argc, char *argv[])
{
  ...
  do {
    ...
    retval = mtlk_container_run(&drvhlpr.container, &close_evt);
    if (retval == MTLK_ERR_OK) {
        cntr = 0;
    } else {
        ELOG_D("Error during mtlk_container_run (%d)", retval);

        /* wait up to 100 times by 10 ms (up to 1 sec total) */
        if (++cntr > 100) {
            ELOG_V("Errors limit is exceeded");
            break;
        }
        usleep (10 * 1000);
    }
