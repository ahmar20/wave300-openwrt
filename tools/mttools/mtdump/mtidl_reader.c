...
_mtlk_request_mtidl_item_clb(const mtlk_mtidl_item_t* item, mtlk_handle_t ctx)
{
  ...
  if(_MTIDL_IS_REQUESTABLE(item->type) && !strcasecmp(req_ctx->friendly_name, item->friendly_name))
  {
    ...
    {
      mtlk_print_mtidl_item(req_ctx->mtidl_path, item->binary_type, item->info_source, item_buffer, size);
    }
...
int __MTLK_IFUNC
mtlk_request_mtidl_item(const char* mtidl_path, const char* ifname, const char* friendly_name, void* provider_id, uint16 info_index)
{
  int res;
