...
mtlk_log_new_pkt_reserve_multi(uint32 pkt_size, mtlk_log_multi_buffer_t **ppdata)
{
  ...
  uint32 nof_buffers = (pkt_size / buf_size) + 3; /* first part + <n> full buffers + last part + null buffer */
  mtlk_log_buf_entry_t *ready_buf = NULL;
  int res = TRUE;

  printk("RESERVE: pkt_size='%u', nof_buffers='%u'\n", pkt_size, nof_buffers);
  ...
  while (pkt_size > 0) {
    ...
    printk("RESERVE: pkt_size='%u'\n", pkt_size);
    if (current_buffer == *ppdata                       /* if this is first packet part */
        && log_data.unsafe_active_buf                   /* and active buffer exists */
        && log_data.unsafe_active_buf->refcount == 0) { /* and active buffer is not in use */
      if (buf_space_left(log_data.unsafe_active_buf)    /* then if space left is enough for signal message header */
        ...
        printk("RESERVE: USE LEFT SPACE buf='%x' part_size='%u'\n", (unsigned int)buf, part_size);
      } else { /* mark buffer as ready and get new free buffer */
        ready_buf = log_data.unsafe_active_buf;
        log_data.unsafe_active_buf = NULL;
        printk("RESERVE: SKIP BUFFER\n");
        continue;
      }
    } else {
      if (!_mtlk_log_unsafe_active_buf_alloc()) {
      ...
      printk("RESERVE: GET NEW BUFFER buf='%x' part_size='%u'\n", (unsigned int)buf, part_size);
    }
    printk("RESERVE: current_buffer='%x'\n", (unsigned int)current_buffer);
    current_buffer->buf       = buf;
    printk("RESERVE: cur->buf='%x'\n", (unsigned int)current_buffer->buf);
    current_buffer->data      = _mtlk_log_buf_get_data_buffer(buf) + buf->data_size;
    printk("RESERVE: cur->data='%x'\n", (unsigned int)current_buffer->data);
    current_buffer->data_size = part_size;
    printk("RESERVE: cur->buf->size='%u'\n", current_buffer->data_size);
    buf->refcount            += 1;
    buf->data_size           += part_size;
    printk("RESERVE: cur->buf->size='%u'\n", current_buffer->buf->data_size);
...
mtlk_log_new_pkt_release_multi(mtlk_log_multi_buffer_t *ppdata)
{
  ...
  while (current_buffer->buf) {
    ...
    printk("RELEASE: buf='%x'\n", (unsigned int)buf);

    ASSERT(buf->refcount >= 1);
    --buf->refcount;
    if (buf != log_data.unsafe_active_buf) {
      printk("RELEASE: FREE BUFFER\n");
