...
/*
  Retrieve character device node handle by minor number
                       ...
                       mtlk_handle_t             ctx)
{
  ...
  do { /* try */
    if (mtlk_dlist_size(&cd->nodes_list) >= cd->cfg.max_nodes) {
      ELOG_D("Can't add node - MAX nodes limit reached (%d)", cd->cfg.max_nodes);
      err = MTLK_ERR_NO_RESOURCES;
      break; /* throw */
    }

    /* find first unused minor number for new node */
    for (node->minor = 0; node->minor < cd->cfg.max_nodes; node->minor++) {
      if (NULL == _cdev_node_get(cd, node->minor))
        break;
    }

    if (node->minor >= cd->cfg.max_nodes) {
