...
#else
  res = register_chrdev(mtlk_major, chrdev_mtlk_log, &mtlk_log_fops);
  if (res < 0)
    return MTLK_ERR_NO_RESOURCES;
...
mtlk_log_on_init(void)
{
  ...
  MTLK_INIT_TRY(logdrv_osdep, MTLK_OBJ_PTR(&log_data_osdep))
...
#else
    log_data_osdep.pnet_procfs_dir->uid.val = 0;
    log_data_osdep.pnet_procfs_dir->gid.val = 0;
#endif
#endif

#if LINUX_VERSION_CODE < KERNEL_VERSION(3,10,0)
    MTLK_INIT_STEP_EX(logdrv_osdep, LOGDRV_PROC_DEBUG, MTLK_OBJ_PTR(&log_data_osdep),
                      create_proc_entry, (proc_entry_debug, S_IFREG|S_IRUSR|S_IRGRP, log_data_osdep.pnet_procfs_dir),
...
#else
    log_data_osdep.pdebug_entry->uid.val = 0;
    log_data_osdep.pdebug_entry->gid.val = 0;
#endif
    ...
    MTLK_INIT_STEP_EX(logdrv_osdep, LOGDRV_PROC_STAT, MTLK_OBJ_PTR(&log_data_osdep),
                      create_proc_entry, (proc_entry_statistics, S_IFREG|S_IRUSR|S_IRGRP, log_data_osdep.pnet_procfs_dir),
...
#else
    log_data_osdep.pstat_entry->uid.val = 0;
    log_data_osdep.pstat_entry->gid.val = 0;
#endif
